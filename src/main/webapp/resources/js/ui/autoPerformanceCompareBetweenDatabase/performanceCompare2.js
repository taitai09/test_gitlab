	...
	$('#submit_form #project_id').combobox({
		...
		onSelect: function(data) {
			...
			$("#submit_form #original_dbid").combobox({
				url:"/AutoPerformanceCompareBetweenDatabase/loadOriginalDb?project_id="
					+data.project_id+"&database_kinds_cd="+$("#database_kinds_cd").val(),
				method:"get",
...
//					$("#submit_form #perf_check_target_dbid").combobox('reload','/AutoPerformanceCompareBetweenDatabase/loadOriginalDb?project_id='+data.project_id+'&original_dbid=0');
				},
				onSelect: function(data) {
					if(data.original_dbid != null && data.original_dbid != "") {
						// 대상 DB
						$("#submit_form #perf_check_target_dbid").combobox({
							url:"/AutoPerformanceCompareBetweenDatabase/loadOriginalDb?project_id="+data.project_id
	...
	$("#perf_compare_meth_cd").combobox({
		...
		textField:'cd_nm',
		onLoadSuccess:function( item ) {
			$("#perf_compare_meth_cd").combobox("setValue",item[0].cd);
		},
...
function loadSqlPerformanceCombo( projectId ) {
	let dbcd = $("#database_kinds_cd").val();
	// submit_form SQL점검팩 콤보
	$('#submit_form #sqlPerformanceP').combobox({
		url:"/AutoPerformanceCompareBetweenDatabase/getSqlPerfPacName?project_id="+projectId+"&database_kinds_cd="+dbcd,
		method:"post",
...
var callback_readSqlPerfDetail = function(value) {
	...
	if ( data.perf_compare_meth_cd != null && data.perf_compare_meth_cd != '' ){
		...
		$("#submit_form #perf_compare_meth_cd").combobox("setValue", "1");
	}
...
//var callback_performanceCompareCallAction = function (result) {
...
//if (parent.openMessageProgress != undefined) parent.openMessageProgress("수행결과 조회"," ");
	...
	ajaxCall("/AutoPerformanceCompareBetweenDatabase/loadPerformanceList",
			$("#submit_form"),
			"POST",
...
function createList() {
	$("#tableList").datagrid({
		...
		columns:[[
			...
			{field:'asis_db_name',title:'ASIS<br>DB',width:'6%',halign:'center',align:'center',rowspan:'2'},
			{field:'tobe_db_name',title:'TOBE<br>DB',width:'6%',halign:'center',align:'center',rowspan:'2'},
