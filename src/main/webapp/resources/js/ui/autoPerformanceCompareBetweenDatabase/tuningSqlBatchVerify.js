...
//	$(".sql_time_limt_cd .textbox").focusout(function() {
...
function sqlPerformancePac( projectId ) {
	...
	$('#submit_form #sqlPerformanceP').combobox({
		url:"/AutoPerformanceCompareBetweenDatabase/getTobeSqlPerfPacName?project_id="+projectId
...
function createList() {
	...
	$("#tableNorthList").datagrid({
		...
		columns:[[
			...
			{field:'plan_change_cnt',title:'Plan 변경',width:'5%',halign:'center',align:'right',rowspan:2},
...
//			openExplainPlan();
	...
	$("#tableSouthList").datagrid({
		...
		columns:[[
			...
			{title:'일괄검증결과',halign:'center',align:'center',colspan:5},
			{title:'ASIS',halign:'center',align:'center',colspan:4},
			{title:'TOBE',halign:'center',align:'center',colspan:3},
			{title:'TUNING',halign:'center',align:'center',colspan:2},
			{title:'일괄검증',halign:'center',align:'center',colspan:3,id:'colorId3'},
			...
			{field:'plan_change_yn',title:'Plan<br>변경여부',width:'4%',halign:'center',formatter:setValueNull,styler:cellPlanStyler,align:'center'},
...
function ajaxCallNorthTableList() {
	...
	ajaxCall("/AutoPerformanceCompareBetweenDatabase/loadTuningBatchValidationNorthList",
			$("#submit_form"),
			"POST",
...
function ajaxCallSouthTableList() {
	...
	/* Tablespace 한계점 예측 - 상세 리스트 */
	ajaxCall("/AutoPerformanceCompareBetweenDatabase/loadTuningBatchValidationSouthList",
			$("#submit_form"),
			"POST",
...
function Btn_LoadPerformanceCheckCount() {
	...
	ajaxCall("/AutoPerformanceCompareBetweenDatabase/loadTuningPerfResultCount",
			$("#submit_form"),
			"POST",
...
//		sqlPerformancePac( $('#submit_form #project_id').val() );
...
function Btn_CollectiveVerification() {
	...
	parent.$.messager.confirm( param,msgStr,function(r) {
		if (r) {
			...
			ajaxCall("/AutoPerformanceCompareBetweenDatabase/countTuningEndTms",
					$("#submit_form"),
					"POST",
					callback_countTuningEndAction);
			
//			ajaxCall("/AutoPerformanceCompareBetweenDatabase/getTuningVerifyTargetCount",
//					$("#submit_form"),
//					"POST",
...
//			alert( sqlTimeLimt_cd + " , "+$("#sql_time_direct_pref_value").val() +", "+$("#sql_time_limt_cd").combobox("getText"));
...
//		parent.$.messager.alert('정보','튜닝대상으로 선정되었던 SQL이 존재합니다.<br>튜닝대상 선정 작업이 수행되었을 경우 일괄검증을 재수행 할 수 없습니다.','info');
	}
}
	...
	if ( result.txtValue == 'false' ) {
		...
		parent.$.messager.confirm(param,msgStr,function(r) {
			if (r) {
				...
				ajaxCall("/AutoPerformanceCompareBetweenDatabase/forceUpdateTuningSqlAutoPerf",
						$("#submit_form"),
						"POST",
...
function updateTuningSqlAutoPerformance() {
	// 02. SQL 자동성능점검 수행
	ajaxCall("/AutoPerformanceCompareBetweenDatabase/updateTuningSqlAutoPerformance",
			$("#submit_form"),
			"POST",
...
function showTuningReqPopup(){
	...
	if ( rows.length > 0 ) {
		...
		if ( $("#assign_form #isAll").val() == "A" ) {
			ajaxCall("/AutoPerformanceCompareBetweenDatabase/loadTuningBatchValidationSouthListAll",
					$("#submit_form"),
					"POST",
...
function dataListCall( data ) {
	...
	if ( data.length > 0 ) {
		...
		ajaxCall("/AutoPerformanceCompareBetweenDatabase/Popup/insertSelectedTuningTarget",
				$("#assign_form"),
				"POST",
...
function performedCheck( callBackFunction ) {
	// 수행중 & 수행한 내역 조회
	ajaxCall("/AutoPerformanceCompareBetweenDatabase/countTuningExecuteTms",
			$("#submit_form"),
			"POST",
			callBackFunction );
}
...
function Excel_Download_North() {
	...
	$("#submit_form").attr("action","/AutoPerformanceCompareBetweenDatabase/excelNorthDownload");
	$("#submit_form").submit();
...
function Excel_Download_South() {
	...
	$("#submit_form").attr("action","/AutoPerformanceCompareBetweenDatabase/excelSouthDownload");
	$("#submit_form").submit();
	$("#submit_form").attr("action","");
}
...
function sqlPerfPacReload( projectId, sqlPerfId ) {
	...
	var url = "/AutoPerformanceCompareBetweenDatabase/getTobeSqlPerfPacName?project_id="+projectId
	+"&database_kinds_cd="+$("#database_kinds_cd").val();
