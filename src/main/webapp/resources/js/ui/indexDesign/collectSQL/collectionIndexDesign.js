	...
	$('#selectCombo').combobox({
		...
		onSelect:function(rec){
			...
			$("#selectExecSeq").combobox({
				...
				panelHeight: 500,
				onSelect:function(rec){
//					console.log("확인:::"+rec.access_path_value);
...
//			});
//			
			if($("#access_path_value").val() != '') {
				$("#selectExecSeq").combobox("setValue",$("#access_path_value").val().split("|")[0]);
				$("#submit_form #exec_seq").val($("#access_path_value").val().split("|")[0]);
				$("#strStartDt").textbox("setValue",$("#access_path_value").val().split("|")[1]);
				$("#strEndDt").textbox("setValue",$("#access_path_value").val().split("|")[2]);
				$("#analysis_sql_cnt").textbox("setValue",$("#access_path_value").val().split("|")[3]);
				$("#access_path_exec_dt").textbox("setValue",$("#access_path_value").val().split("|")[4]);
...
//		onLoadError: function(){
//			parent.parent.$.messager.alert('','테이블 조회중 오류가 발생하였습니다.');
//			return false;
//		}
//	});	
...
//			{field:'blocks',title:'BLOCKS',halign:"center",align:"right",formatter:formatPrice,sortable:"true"},			
	...
	$("#sqlTextList").datagrid({
		...
		onClickRow : function(index,row) {
			...
			// SQL Information 탭 생성..
//			createSQLInfoTab("accessPathTabs", $("#dbid").val(), row.sql_id, row.plan_hash_value);
			createSQLInfoVSQLAllTab("accessPathTabs", $("#dbid").val(), row.sql_id, row.plan_hash_value);
		},			
		columns:[[
			{field:'rnum',title:'NO',halign:"center",align:"center",sortable:"true"},
...
function setExecValue(selValue){
	...
	$("#submit_form #exec_seq").val(txData[0]);
...
function checkedDuplicate(rows) {
	...
	for(var index = 0; index < rows.length; index++) {
		$('#indexDesignList').datagrid('endEdit', index);
...
var callback_InsertIndexDesignAction = function(result) {
	if(result.result){
		...
		parent.parent.$.messager.alert('',result.message);
	}
};
...
function showIndexAutoDesign(){
	...
	if($('#selectExecSeq').combobox('getValue') == ""){
		parent.parent.$.messager.alert('','분석 회차를 선택해 주세요.');
		return false;
	}
	...
	$("#indexAutoDesign_form #exec_seq").val($('#submit_form #exec_seq').val());
...
function Excel_Down_Table_List(){	
	...
	if($('#selectTableName').textbox('getValue') == '') {
		tempTableName = $('#table_name').val();
		$('#table_name').val('');
	}
