...
//				$(this).combobox('select', items[0][opts.valueField]);
	...
	$("#tableList").datagrid({
		...
		columns:[[
			...
			{field:'app_cnt',title:'APP수',halign:"center",align:'right',formatter:getNumberFormat,sortable:"true"},
...
//        			//$("input:checkbox[name='chk']").eq(node).prop("checked",false);
//        			return false;
//        		}
//        }
	});
...
//	$("#selectCombo").combobox("setValue",$("#wrkjob_cd").val());
	...
	if(callFromParent == "Y" || callFromChild == "Y"){
		ajaxCallApplicationCheck();	
	}
	
});

function ajaxCallApplicationCheck(){
	$('#tableList').datagrid("loadData", []);
	
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("애플리케이션 성능점검"," "); 
	ajaxCall("/ApplicationCheckAction", $("#submit_form"), "POST", callback_ApplicationCheckActionAddTable);	
}
//callback 함수
var callback_ApplicationCheckActionAddTable = function(result) {
...
function requestPerformanceCheck(){
	...
	if(rows != null && rows != ""){
		...
		ajaxCall("/ApplicationRequestCheck",
				$("#submit_form"),
				"POST",
				null);
...
function showAlert(){
	parent.$.messager.alert('','애플리케이션 성능점검요청 수행 시간이<br/>오래걸려 서버에서 실행중입니다.','info',function(){
		setTimeout(function() {
			Btn_OnClick();
		},2000);
	});
