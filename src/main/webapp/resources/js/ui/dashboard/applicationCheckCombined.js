$(document).ready(function() {
	$("#tableList").treegrid({
		...
		onClickRow : function(row) {
        	var menuId = "102";
        	var menuNm = "애플리케이션 진단";
        	var menuUrl = "/ApplicationDiagnostics/Summary";
        	var menuParam = "";			        	
            
            menuParam = "wrkjob_cd="+row.wrkjob_cd+"&gather_day="+$("#gather_day").datebox("getValue")+"&call_from_parent=Y";
...
function Btn_OnClick(){
	...
	ajaxCall("/DashboardV2/getApplicationCheckCombined", $("#submit_form"), "POST", callback_ApplicationCheckCombined);
	//ajaxCall("/DashboardV2/getApplicationCheckCombinedResult", $("#submit_form"), "POST", callback_ApplicationCheckCombinedResult);
}

//callback 함수
var callback_ApplicationCheckCombined = function(result) {
	treegrid_json_string_callback_common(result,'#tableList',false);
};

var callback_ApplicationCheckCombinedResult = function(result) {
