...
$(document).ready(function() {
	...
	$("#delayList").datagrid({
		...
		columns:[[
			...
			{field:'program_type_cd_nm',title:'프로그램유형',halign:"center",align:'center',sortable:"true"},
			{field:'batch_work_div_cd_nm',title:'배치작업구분',halign:"center",align:'center',sortable:"true"},
			{field:'tuning_complete_due_dt',title:'튜닝기한일',width:"100",halign:"center",align:'center',sortable:"true",formatter:getDateFormatTuner},
			{field:'wrkjob_peculiar_point',title:'업무특이사항',halign:"center",align:'left',sortable:"true"},
			{field:'tr_cd',title:'소스파일명(Full Path)',width:'350px',halign:"center",align:'left',sortable:"true"},
			{field:'dbio',title:'SQL식별자(DBIO)',width:'350px',halign:"center",align:'left',sortable:"true"},
			{field:'module',title:'MODULE',width:"150",halign:"center",align:'left',sortable:"true"}
	...
	$("#progressTab").tabs({
		...
		onSelect: function(title,index){
			...
			if ( index == 0) {
				parent.openMessageProgress("튜닝 작업 대기 조회"," ");
				ajaxCallTuningWaitJob();
			}else if(index == 1){
				parent.openMessageProgress("튜닝 진행 조회"," ");
				ajaxCallTuningProgress();
			}else if(index == 2){
				parent.openMessageProgress("튜닝 지연 예상 조회"," ");
				ajaxCallTuningExpectedDelay();
			}else if(index == 3){
				parent.openMessageProgress("튜닝 지연 조회"," ");
				ajaxCallTuningDelay();
			}else{
				parent.openMessageProgress("주간 튜닝 완료 조회"," ");
...
//날짜형식 변경
function getDateFormatTuner(val, row) {
    var newDate = "";
...
function fnSearch(){
	let tab = $("#progressTab").tabs('getSelected');
	let tabIdx = $("#progressTab").tabs('getTabIndex',tab);
...
function callback_List(result , listId ) {
	var dataLength = JSON.parse(result).dataCount4NextBtn;
	json_string_callback_common(result, listId ,true);
