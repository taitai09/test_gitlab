...
//		view: myview,
		...
		columns:[[
			...
			{field:'value',title:'VALUE',halign:"center",align:'center'},
			{field:'parameter_chg_dt',title:'PARAMETER_CHG_DT',halign:"center",align:'center'}
		]],
...
function Btn_OnClick(){
	...
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("파라미터 최근변경 내역"," "); 
	
	ajaxCall("/ParameterRecentChanges/Chart",
		$("#submit_form"),
		"POST",
...
function showDetailList(){
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("파라미터 최근변경 내역 - 상세"," "); 
	
	ajaxCall("/ParameterRecentChanges/Detail",
		$("#submit_form"),
		"POST",
...
var callback_DetailListAction = function(result) {
	...
	treegrid_json_string_callback_common(result,'#tableList',true);
};