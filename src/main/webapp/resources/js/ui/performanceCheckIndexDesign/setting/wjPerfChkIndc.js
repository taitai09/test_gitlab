...
//	$(function(){
//		$('#pass_max_value').bind('focus',function(){
...
function getWjPerfChkIndc(){
	...
	$("#tableList").datagrid({
			...
			columns:[[
				...
				{field:'perf_check_indc_nm',title:'점검지표',width:"10%",halign:"center",align:'left'},
				...
				{field:'perf_check_meth_cd_nm',title:'점검 방법',width:"4%",halign:"center",align:'center'},
...
function Btn_OnClick(){
	...
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("업무별 성능 점검 지표 기준값 관리"," ");
...
function Btn_SaveSetting(){
		...
		if($("#perf_check_indc_id").combobox('getValue') == ""){
			parent.$.messager.alert('','점검 지표를 선택해 주세요.');
			return false;
		}
...
function Btn_DeleteSetting(){
	if($("#temp_wrkjob_cd").combobox("getValue") ==='' || $("#temp_wrkjob_cd").combobox("getValue") === null ){
		...
		parent.$.messager.confirm('', '[ 점검지표 : ' + $("#perf_check_indc_id").combobox("getText")+ ', <br/>프로그램 : '+ $("#perf_check_program_div_cd").combobox("getText")+' ]' + ' 을(를)<br/>삭제 하시겠습니까?', function(check) {
			if (check) {
...
var callback_DeleteSettingAction = function(result) {
	if(result.result){
		parent.$.messager.alert('','[ 점검지표 : ' + $("#perf_check_indc_id").combobox("getText")+ ', <br/>프로그램 : '+ $("#perf_check_program_div_cd").combobox("getText")+' ]'+' 이(가) 성공적으로 삭제되었습니다.','info',function(){
			setTimeout(function() {
				Btn_OnClick();
				Btn_ResetField();
			},1000);	
...
//	$("#detail_form #not_pass_pass").textbox({readonly: true});
