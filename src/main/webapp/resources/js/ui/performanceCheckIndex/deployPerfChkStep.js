...
//			
//			if(set_first == 0){
//				$("#old_del_yn").val(newvalue);
//			}
//			set_first += 1;
//			console.log(set_first)
//			
//		},
//	});
...
function getDeployPerfChkStep(){
	...
	$("#tableList").datagrid({
			...
			columns:[[
				...
				{field:'perf_check_auto_exec_yn',title:'성능점검자동 실행여부',width:"10%",halign:"center",align:'center'},
				{field:'perf_check_step_desc',title:'점검 단계 설명',width:"38%",halign:"center",align:'left'},
...
function Btn_SaveSetting(){
		...
		if($("#perf_check_step_nm").textbox('getValue') == ""){
			parent.$.messager.alert('','점검단계를 입력해 주세요.');
			return false;
		}
		
		if($("#perf_check_step_nm").textbox("getText").length > 100){
			parent.$.messager.alert('','점검단계의 내용을 100자 이내로 입력해주세요.');
			return false;
		}
		
		if($("#perf_check_auto_exec_yn").combobox('getValue') == ""){
			parent.$.messager.alert('','성능점검자동 실행여부를 선택해 주세요.');
			return false;
		}
		...
		if($("#perf_check_step_desc").val() == ""){
			parent.$.messager.alert('','점검 단계 설명을 입력해 주세요.');
			return false;
		}	
		if($("#perf_check_step_desc").val().length > 200){
			parent.$.messager.alert('','점검 단계의 설명의 내용을 200자 이내로 입력해주세요.');
			return false;
		}
