...
//				url:"/PerformanceCheckIndex/getPerfCheckStep?search_wrkjob_cd="+rec.id,
//				method:"get",
...
/*	// 예외요청처리상태
	...
	$('#search_exception_prc_meth_cd').combobox({
		url:"/Common/getCommonCode?grp_cd_id=1061&isAll=Y",
		method:"get",
		valueField:'cd',
		textField:'cd_nm',
		onLoadError: function(){
...
//		url:"/Common/getCommonCode?grp_cd_id=1052&isAll=Y",
	...
	$('#exec_request_form #exception_request_why_cd').combobox({
		url:"/PerformanceCheckIndex/getCommonCode?grp_cd_id=1060",
		method:"get",
		valueField:'cd',
		textField:'cd_nm',
		onChange:function(val){
...
//	    	console.log("rec.ref_vl_1:",rec.ref_vl_1);
	...
	$('#exec_request_form #exception_prc_meth_cd').combobox({
		url:"/Common/getCommonCode?grp_cd_id=1061&isChoice=Y",
		method:"get",
		valueField:'cd',
		textField:'cd_nm',
		onChange:function(val){
...
//	// 예외요청 상세사유
	...
	$(".detailT #sql_command_type_cd").textbox("readonly","true");
...
//				},
				...
				columns:[[
					...
					{field:'perf_check_step_nm',title:'성능점검단계',width:"8%",halign:"center",align:'center'},
					{field:'deploy_check_status_cd_nm',title:'성능 점검 상태',width:"6%",halign:"center",align:'center'},
					...
					{field:'perf_check_auto_pass_del_yn',title:'점검제외삭제여부',width:"6%",halign:"center",align:'center'},
...
//					{field:'program_type_cd_nm',title:'프로그램유형',width:"8%",halign:"center",align:'center'},  //임시
					...
					{field:'perf_check_step_id',hidden:true},//성능점검단계
...
/*function getDeployPerfChkDetailResult(){
	...
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("성능 점검 예외 요청 - 예외 처리"," ");
	
	ajaxCall("/PerformanceCheckIndex/DeployPerfChkDetailResult",
			$("#detail_form"),
			"POST",
...
/*********** 성능점검결과구분코드 (PERF_CHECK_RESULT_DIV_CD) **********
| 	A : 적합		|	B : 부적합	|	C : 오류		|	9 : 미수행	|
******************************************************************/
	
/******************* 배포성능점검기본코드 (DEPLOY_CHECK_STATUS_CD) ********************
| 00 : 개발확정 | 01 : 점검중 | 02 : 점검완료 | 03 : 배포완료 | 04 : 배포취소 | 05 : 배포삭제	 |
**********************************************************************************/
...
*************************************************/
	...
	if($("#submit_form #user_auth_id").val() != '4' && $("#submit_form #user_auth_id").val() != '5' && $("#submit_form #user_auth_id").val() != '6'){
		...
		$("#exec_request_form #btn_reset").linkbutton({disabled:true});
			...
			//[개발자(O) && 자신요청(O)] 개발자(O) 업무개발_배포성능관리자(O) 배포성능관리자(O) 점검완료(O)
...
/*function getDeployPerfChkPlantable(){
	...
	ajaxCall("/PerformanceCheckIndex/DeployPerfChkPlanTableList",
			$("#detail_form"),
			"POST",
...
function getPerfCheckResultTableList(){
	$("#perfCheckResultList").datagrid({
		view: myview,
		columns:[[
			{field:'perf_check_indc_nm',title:'점검 지표',width:"18%",halign:"center",align:'center'},
...
//			{field:'indc_yn_decide_div_cd_nm',title:'여부값 판정구분',width:"14%",halign:"center",align:'center'},
			{field:'exec_result_value',title:'성능 점검 결과값',width:"10%",halign:"center",align:'center'},
			{field:'perf_check_result_div_cd_nm',title:'성능 점검 결과',width:"10%",halign:"center",align:'center', styler:cellStyler2},
			{field:'except_reg_yn',title:'예외등록여부',width:"10%",halign:"center",align:'center'},
			{field:'perf_check_result_desc',title:'성능점검 결과내용',width:"33%",halign:"center",align:'left'},
...
/*sql타입에 따라 ExecutionPlan/예상 실행계획 보이기*/
function showExecutionPlan(){
	
	$("#tr_exec_plan").hide();			//Execution Plan
	$("#tr_perf_chk_result").hide();	//예상실행계획
	$(".detailT3-wrapper").hide();		//table 감싸주는 wrapper
	
	let program_type_cd_nm = $("#detail_form #program_type_cd_nm").val();					 /* 온라인 / 배치 */
	let sql_command_type_cd = $("#detail_form #sql_command_type_cd_input").val();					 /* select / 나머지 dml */
	let perf_check_result_div_cd_nm = $("#detail_form #perf_check_result_div_cd_nm").val();	 /* 적합 / 부적합 / 미수행 */
	console.log("program_type_cd_nm:"+program_type_cd_nm+" sql_command_type_cd:"+sql_command_type_cd
			+"perf_check_result_div_cd_nm:"+perf_check_result_div_cd_nm);
	
	
	if(perf_check_result_div_cd_nm != "미수행"){
		if(sql_command_type_cd == "SELECT"){
			//select문일 때							//select문일 때
			$("#tr_exec_plan").show();				//Execution Plan show
			$("#tr_perf_chk_result").hide();		//예상실행계획 hide
			$(".detailT3-wrapper").show();			//table 감싸주는 wrapper
			
		}else {
			if(sql_command_type_cd){
				//select제외 dml일 때					//select제외 dml일 때
				$("#tr_exec_plan").hide();			//Execution Plan
				$("#tr_perf_chk_result").show();	//예상실행계획
				$(".detailT3-wrapper").show();		//table 감싸주는 wrapper
				
			}else {
				//처음 페이지 로드되어 정보가 없을 때			//처음 페이지 로드되어 정보가 없을 때
				$("#tr_exec_plan").hide();			//Execution Plan
				$("#tr_perf_chk_result").hide();	//예상실행계획
				$(".detailT3-wrapper").hide();		//table 감싸주는 wrapper
			}
...
//	alert($("#detail_form #user_auth_id").val());
	...
	ajaxCall("/PerformanceCheckIndex/ProPerfExcReq",
			$("#submit_form"),
			"POST",
...
//	alert($("#exec_request_form #exception_prc_meth_cd").combobox("getValue"));
	...
	if(($("#detail_form #perf_check_result_div_cd").val() == "C" || $("#detail_form #perf_check_result_div_cd").val() == "9") && $("#exec_request_form #exception_prc_meth_cd").combobox("getValue") == "1"){
		parent.$.messager.alert('','성능점검 결과가 [오류] 또는 [미수행] 일경우<BR/>지표단위로 예외요청 할 수 없습니다.');
		return false;
	}
		...
		if($("#howToWay").val() == 'A'){
			...
			if($("#detail_form #perf_check_result_div_cd").val() == "9"){
				parent.$.messager.confirm('', '성능점검결과가 미수행건입니다. <br/>그래도 예외요청을 하시겠습니까?', function(check) {
					if(check){
						ajaxCall("/PerformanceCheckIndex/DeployPerfChkExcptRequest/CheckExceptionPrcStatusCd2",$("#detail_form"),"POST",callback_checkExceptionPrcStatusCdAction);	
					}
				});
			}else{
		...
		//일괄 예외요청
			...
			for(var i = 0; i < rows.length; i++){
				...
				//미수행건이 있다면 한번 물어봄.
					parent.$.messager.confirm('', '성능점검결과가 미수행건이 존재합니다. <br/>그래도 예외요청을 하시겠습니까?', function(check) {
						if(check){
...
//			exception_prc_meth_cd.push(rows[i].exception_prc_meth_cd);
		...
		//보낼 파라미터를 저장
		$("#detail_form #program_nm").textbox("setValue",program_nm.join(","));
		$("#dbio").textbox("setValue",dbio.join(","));
		$("#deploy_id").textbox("setValue",deploy_id.join(","));
		
		$("#perf_check_id").val(perf_check_id.join(","));
		$("#program_id").textbox("setValue",program_id.join(","));
...
/*		console.log("넣은값 program_nm :::"+$("#program_nm").textbox("getValue"));
		...
		ajaxCall("/PerformanceCheckIndex/DeployPerfChkExcptRequest/MultiRequest",
				$("#detail_form"),
				"POST",
...
//		console.log(rows[i].exception_prc_meth_cd);
	...
	$("#detail_form #perf_check_id").val(perf_check_id.join(","));
	$("#detail_form #program_nm").textbox("setValue",program_nm.join(","));
	...
	console.log("넣은값 program_nm :::"+$("#detail_form #program_nm").textbox("getValue"));
	...
	ajaxCall("/PerformanceCheckIndex/DeployPerfChkExcptRequest/multiExceptionHandling",
			$("#detail_form"),
			"POST",
...
var callback_MultiExceptionHandlingAction = function(result) {
	...
	if(result.result){
		...
		//없었을시
			...
			parent.$.messager.alert('','일괄 저장이 완료 되었습니다.','info',function(){
				setTimeout(function() {
					Btn_OnClick();
					Btn_ResetFieldAll();
					createExceptionHandlingTab_reload();
...
//			rows = $('#tableList').datagrid('getSelected');
			...
			if($("#detail_form #perf_check_auto_pass_yn").val() == 'Y' && $("#detail_form #exception_prc_meth_cd").val() == '1'){
					parent.$.messager.alert('','성능점검 점검제외에서 지표단위점검으로<br/>변경 요청하셨습니다. 점검제외 삭제이므로 <br/>[성능 점검 예외 삭제] 메뉴를 통해 처리 바랍니다.');
...
function createExceptionHandlingTab(){
...
//	console.log("ifram params:::"+$("#detail_form").serialize());
	document.getElementById('exec_handling').src = "/PerformanceCheckIndex/ExceptionHandling/CreateTab?"+$("#detail_form").serialize();
	
};
function createExceptionHandlingTab_reload(){
	document.getElementById('exec_handling').src = "/PerformanceCheckIndex/ExceptionHandling/CreateTab";
};
...
function setDetailView(selRow){
	...
	$("#detail_form #sql_command_type_cd_input").val(selRow.sql_command_type_cd);
	...
	$("#detail_form #paging_yn").textbox("setValue", selRow.pagingYn);
	$("#detail_form #paging_cnt").textbox("setValue", selRow.pagingCnt);
	...
	if(tab_index == 0){
		...
		console.log("tab2");
		createExceptionHandlingTab();  //탭을 생성한다.
	}
	showExecutionPlan();			// sql타입에 따라 ExecutionPlan/예상 실행계획 보이기
...
function ajaxCallGetExecPlan(){
	...
	let sql_command_type_cd = $("#detail_form #sql_command_type_cd_input").val();
	...
	ajaxCallWithJson("/PerformanceCheckIndex/ProPerfExcReq/getExecPlan",
			jData,
			"POST",
...
function Btn_ResetFieldAll(){
	...
	$("#detail_form #deploy_id").textbox("setValue", "");
	$("#detail_form #deploy_nm").textbox("setValue", "");
	$("#detail_form #deploy_check_status_cd_nm").textbox("setValue", "");
	$("#detail_form #deploy_expected_day").textbox("setValue", "");
	$("#detail_form #program_id").textbox("setValue", "");
	...
	$("#detail_form #program_execute_tms").textbox("setValue", "");
...
function Excel_Download(){
	...
	$("#submit_form").attr("action","/PerformanceCheckIndex/ProPerfExcReq/ExcelDown");
	$("#submit_form").submit();
}
