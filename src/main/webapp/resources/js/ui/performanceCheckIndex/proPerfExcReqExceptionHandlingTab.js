...
//	if($("#exception_prc_why").prop("readonly")){
//		$("#exception_prc_why").textbox({required:false});
//	}else{
//		$("#exception_prc_why").textbox({required:true});
//	}
...
//	if(parent.fnChangeValue(val)){ // (보류 20190307)
		parent.Btn_OnClick();
...
function Btn_ExceptionHandling(){
	...
	var rows = parent.$('#tableList').datagrid('getChecked');
	...
	//일괄 예외처리 (영구,한시점검제외만 가능)
	if($("#howToWay").val() != '' && $("#howToWay").val() == 'C'){
		//일괄 예외처리
		var excepter_id = $("#excepter_id").textbox("getValue");  //input값
		var exception_prc_why = $("#exception_prc_why").textbox("getValue");  //input값 
		
		parent.multiExceptionHandling(excepter_id, exception_prc_why);
	...
	//개별 예외처리
		...
		if($("#exec_handling_form #exception_prc_status_cd").val() == '1' && $("#exec_handling_form #exception_prc_meth_cd").val() == '1'){
			ajaxCall("/PerformanceCheckIndex/DeployPerfChkExcptRequest/exceptionHandling",$("#exec_handling_form"),"POST",callback_exceptionHandlingAction);	
		}else if($("#exec_handling_form #exception_prc_status_cd").val() == '1' && ($("#exec_handling_form #exception_prc_meth_cd").val() == '2' || $("#exec_handling_form #exception_prc_meth_cd").val() == '3')){
			var strText ="";
			
			if($("#exec_handling_form #exception_prc_meth_cd").val() == '2'){
				strText ="영구 점검제외를 선택하셨을 경우 성능점검을 수행하지 않습니다. 해당 프로그램에 대해<br/>영구 점검제외로 저장하시겠습니까?";
			}else if($("#exec_handling_form #exception_prc_meth_cd").val() == '3'){
				strText ="한시 점검제외를 선택하셨습니다. <br/> 해당 프로그램에 대해 한시 점검제외로<br/>저장하시겠습니까?";
			}
			
			parent.$.messager.confirm('', strText, function(check) {
				if (check) {
					ajaxCall("/PerformanceCheckIndex/DeployPerfChkExcptRequest/exceptionHandling",$("#exec_handling_form"),"POST",callback_exceptionHandlingAction);	
				}else{
					return false;
			...
			parent.$.messager.alert('','정의되지 않은 성능점검방법코드입니다.');
			return false;
		}
...
var callback_exceptionHandlingAction = function(result) {
	if(result.result){
		parent.$.messager.alert('','저장이 완료 되었습니다.','info',function(){
			setTimeout(function() {
				//지표단위일 경우에만 성능점검을 수행한다.
				if($("#exec_handling_form #exception_prc_meth_cd").val() == 1){
					if(parent.parent.openMessageProgress != undefined) parent.parent.openMessageProgress("성능점검 수행중"," ");
					ajaxCall("/PerformanceCheckIndex/DeployPerfChkExcptRequest/perfCheckExecute",$("#exec_handling_form"),"POST",callback_perfCheckExecute);	
				}else{
					Btn_reProPerfExecReqTab();  //처리완료 // [ 1 : 요청 2 : 처리완료 3 : 요청취소 4 : 처리반려 ]이었으나 현재는 하드코딩상태로 변경됨.  
				}
//				Btn_onClick();
//				Btn_ResetField();
			},1000);	
...
//callback 함수
var callback_perfCheckExecute = function(result) {
	console.log("result:"+result.result);
	console.log("message:"+result.message);
	parent.parent.$.messager.progress('close');
	Btn_reProPerfExecReqTab();  //처리완료
...
//	}
//	if(checkbox.length == 0){
//		parent.$.messager.alert('','예외를 삭제할 체크된 점검지표가 없습니다.');
//		return false;
//	}
//	setDataToTheForm("exec_remove_form");
//	
//	ajaxCall("/PerformanceCheckIndex/DeployPerfChkExcptRequest/removeException",
//			$("#exec_remove_form"),
//			"POST",
...
function Btn_onClick(){
	parent.document.getElementById('exec_handling').src = "/PerformanceCheckIndex/ExceptionHandling/CreateTab?"+$("#exec_handling_form").serialize();
};
...
function Btn_SettingText(val){
	$('#exec_handling_form #exception_prc_why').textbox("setValue",val); 
}