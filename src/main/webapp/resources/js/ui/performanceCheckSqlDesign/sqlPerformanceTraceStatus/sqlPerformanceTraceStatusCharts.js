...
var callback_BaseDateAction = function(result) {
	...
	let nbsp = "&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;";
	let nbsp2 = "&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;";
	let html = nbsp + 'Plan 변경' + nbsp2 + '<span style="font-weight:bold;">' + finalDistributionDate + '</span></div>';
	
	$('#planChange').html(html);
...
//	createBufferGetsPie();
//	createBufferGetsBar();
//	createElapsedTimePie();
//	createElapsedTimeBar();
//	createPlanChangePie();
//	createPlanChangeBar();
//	createPerformanceCheckSql();
//	createExceptionHandlingSql();
//	createPerformanceCheckSqlPie();
//	createPerformanceCheckSqlBar();
//	createExceptionHandlingSqlPie();
//	createExceptionHandlingSqlBar();
}
...
function createPC(result){
	...
	if(chartPC != "undefined" && chartPC != undefined){
		chartPC.destroy();
	}
	
	Ext.define("chartPC.colors", {	// Label 색상 정의
		singleton:  true,
		COLOR_01: blueColor,	// 일일 부적합률
		COLOR_02: redColor		// 일일 부적합
	});
	
	chartPC = Ext.create("Ext.panel.Panel",{
		width : '100%',
//		height : northBarChartHeight,
		height : southPieChartHeight,
		border : false,
		renderTo : document.getElementById("chartPC"),
		layout : 'fit',
		items : [{
			xtype : 'cartesian',
			border : false,
			...
			axes : [{
				type : 'numeric',
				position : 'left',
				fields: ['plan_change_fail'],
				minimum: 0
			},{
				type : 'numeric',
				position : 'right',
				fields: ['plan_change_fail_ratio'],
				minimum: 0,
			},{
				type : 'category',
				position : 'bottom',
			...
			series : [{
				type : 'bar',
				xField : 'day',
				yField : 'plan_change_fail',
				title: '일일 부적합',
				colors: [chartPC.colors.COLOR_02],
				label: {
					field: 'plan_change_fail',
					display: 'insideEnd',
					orientation: 'horizontal',
					font: '9px "Lucida Grande", "Lucida Sans Unicode", Verdana, Arial, Helvetica, sans-serif',
				}
				...
				xField : 'day',
				yField : 'plan_change_fail_ratio',
				title: '일일 부적합률(%)',
				colors: [chartPC.colors.COLOR_01],
				label: {
					field: 'plan_change_fail_ratio',
					display: 'insideEnd',
					orientation: 'horizontal',
					font: '9px "Lucida Grande", "Lucida Sans Unicode", Verdana, Arial, Helvetica, sans-serif',
				}
...
function createPCPie(result){
	...
	if(chartPCPie != "undefined" && chartPCPie != undefined){
		chartPCPie.destroy();
	}
	...
	for(let index = 0; index < dataLen; index++) {
		pass += Number(data[index].plan_change_pass);
		fail += Number(data[index].plan_change_fail);
	}
	
	Ext.define("chartPCPie.colors", {	// Label 색상 정의
		singleton:  true,
		COLOR_01: greenColor,	// 적합
		COLOR_02: redColor		// 부적합
	});
	
	chartPCPie = Ext.create("Ext.panel.Panel",{
//		width : '100%',
//		height : '100%',
		width : southPieChartWidth,
		height : southPieChartHeight,
		border : false,
		renderTo : document.getElementById("chartPCPie"),
		layout : 'fit',
		items : [{
			xtype : 'polar',
			border : false,
...
//				data : data,
			...
			series : [{
				type : 'pie',
				colors: [chartPCPie.colors.COLOR_01, chartPCPie.colors.COLOR_02],
				angleField : 'data1',
				label: {
					field : 'name',
					display: 'inside'
...
function createPCBar(result){
	...
	if(chartPCBar != "undefined" && chartPCBar != undefined){
		chartPCBar.destroy();
	}
	...
	for(let index = 0; index < dataLen; index++) {
		improved += Number(data[index].plan_change_improve);
		regressed += Number(data[index].plan_change_regress);
	}
	...
	Ext.define("chartPCBar.colors", {	// Label 색상 정의
		singleton:  true,
		COLOR_01: greenColor,
		COLOR_02: redColor
	});
	
	chartPCBar = Ext.create("Ext.panel.Panel",{
//		width : '100%',
//		height : '100%',
		width : southPieChartWidth,
		height : southPieChartHeight,
		border : false,
		renderTo : document.getElementById("chartPCBar"),
		layout : 'fit',
		items : [{
			xtype : 'cartesian',
			border : false,
...
//				data : data
			...
			series : [{
				...
				yField : ['data1','data2'],
				title: ['Improved','Regressed'],
				colors: [chartPCBar.colors.COLOR_01, chartPCBar.colors.COLOR_02],
				label: {
					field: ['data1','data2'],
					display: 'insideEnd',
					orientation: 'horizontal',
...
function getChartData() {
	...
	chartUrl.push("/SqlPerformanceTraceStatusChart/chart");
	chartUrl.push("/SqlPerformanceTraceStatusChart/chartPerformance01");
	chartUrl.push("/SqlPerformanceTraceStatusChart/chartPerformance02");
	chartUrl.push("/SqlPerformanceTraceStatusChart/chartException01");
	chartUrl.push("/SqlPerformanceTraceStatusChart/chartException02");
