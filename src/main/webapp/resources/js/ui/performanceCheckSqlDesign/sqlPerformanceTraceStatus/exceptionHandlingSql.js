...
var callback_BaseDateAction = function(result) {
	...
	let html = '<div class="datagrid-cell-group">' + nbsp + '수행시간(초)' + nbsp2 + '<span style="font-weight:bold;">' + finalDistributionDate + '</span></div>';
	
	$('#datagrid-td-group3-0-5').html(html);
...
function ajaxCallTreeList(){
	ajaxCall("/ExceptionHandlingSql/loadExceptionHandlingSql",
			$("#submit_form"),
			"POST",
...
function Excel_Download(){
	...
	$("#submit_form").attr("action","/ExceptionHandlingSql/loadExceptionHandlingSqlExcelDown");
	$("#submit_form").submit();
