...
function Btn_SaveSetting(){
	...
	if($("#perf_check_step_id").combobox('getValue') == ""){
		parent.$.messager.alert('','검증단계를 선택해 주세요.');
		return false;
	}
...
var callback_SaveSettingAction = function(result) {
	if(result.result){
		...
		if(result.txtValue == 'Y'){  //배포성능 검증중인 건수가 존재해서 업데이트를 체크할지 여부물어봐야함.
			parent.$.messager.confirm('', '배포성능 검증중인 건수가 존재합니다. '+
					'스키마를 변경할 경우 배포성능 검증이 정상적으로 종료되지 않을 수 있습니다. ' +
					'스키마를 변경하시겠습니까?', function(check) {
				if(check){
					saveDeployPerfChkParsingSchema("Y");
				}
			});
		}else{  //N 일경우는 배포성능 검증중인 건수가 존재하지않아서 물어보지않고 업데이트 성공
