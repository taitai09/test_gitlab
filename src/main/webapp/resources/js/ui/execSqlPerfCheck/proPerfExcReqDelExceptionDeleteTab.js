...
//	}
	...
	if($("#exec_handling_form #exception_prc_status_cd").val() == '1' && $("#exec_handling_form #perf_check_auto_pass_yn").val() == 'Y' &&$("#exec_handling_form #exception_prc_meth_cd").val() == '1'){
		parent.$.messager.confirm('', '예외 처리 방법이 점검제외에서 지표단위점검으로 변경됩니다. 해당 프로그램에 대해<br/>지표단위점검으로 저장하시겠습니까?', function(check) {
			if (check) {
				ajaxCall("/ExecPerformanceCheckIndex/DeployPerfChkExcptRequest/exceptionDelete",$("#exec_handling_form"),"POST",callback_exceptionHandlingAction);	
			}else{
				return false;
			}
		});
...
function Btn_onClick(){
	parent.document.getElementById('exec_handling').src = "/ExecPerformanceCheckIndex/ExceptionDelete/CreateTab?"+$("#exec_handling_form").serialize();
};
