...
function getProPerfExecReq(){
	...
	$("#tableList").datagrid({
			...
			columns:[[
				...
				{field:'perf_check_auto_pass_del_yn',title:'검증제외삭제여부',width:"8%",halign:"center",align:'center'},
...
function fnGoPrevOrNext(direction){
	...
	createExceptionStateTab_reload();
...
function fnSearch(){
	...
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("성능 검증 예외 처리 이력"," ");
...
//	alert($("#detail_form #user_auth_id").val());
	...
	ajaxCall("/ExecPerformanceCheckIndex/ProPerfExcReqState",
			$("#submit_form"),
			"POST",
...
function formValidationCheck(){
	...
	if($("#submit_form #search_from_exception_reqeust_dt").combobox('getValue') == '' && $("#submit_form #search_to_exception_reqeust_dt").combobox('getValue') != '' ){
		parent.$.messager.alert('','예외처리요청일자를 정확히 입력해 주세요.');
		return false;
	}
	if($("#submit_form #search_from_exception_reqeust_dt").combobox('getValue') != '' && $("#submit_form #search_to_exception_reqeust_dt").combobox('getValue') == '' ){
		parent.$.messager.alert('','예외처리요청일자를 정확히 입력해 주세요.');
...
//	console.log("ifram params:::"+$("#detail_form").serialize());
	if($('#tableList').datagrid('getSelections') && $('#tableList').datagrid('getSelections').length === 0){
		...
		document.getElementById('exec_handling').src = "/ExecPerformanceCheckIndex/ExceptionState/CreateTab?"+$("#detail_form").serialize();
	}
};
function createExceptionStateTab_reload(){
	document.getElementById('exec_handling').src = "/ExecPerformanceCheckIndex/ExceptionState/CreateTab";
};
...
function Excel_Download(){
	...
	$("#submit_form").attr("action","/ExecPerformanceCheckIndex/ProPerfExcReqState/ExcelDown");
	$("#submit_form").submit();
...
function Btn_ResetFieldAll(){
	...
	$("#exec_request_form #exception_request_why").textbox("setValue", "");
