...
//	checkValidExec("exec_handling_form"); //나중에 validation 체크안될때 다른방법으로 실행해보기. 일단은 jstl 이용하여 하였음.
	...
	if(parent.name === 'exceptionProcessHistory'){
		$('.popupOpenEle').css('cursor','auto');
	}else{
		$('.popupOpenEle').css('cursor','pointer')
	}
	...
	$(".popupOpenEle").click(function(){
		//예외 처리 이력 , 예외 처리 탭의 경우 팝업 이벤트 제거.
		if(parent.name === 'exceptionProcessHistory'){
			return ;
		}
		...
		parent.popupOpen($('#exec_handling_form'));

	});
	
	if($("#howToWay").val()){
		if($("#howToWay").val() === 'B'){
			$("#btn_except").linkbutton("disable")
...
function Btn_ExceptionHandling(){
	...
	let deploy_id = rows[0].deploy_id
	
	$("#exec_handling_form #deploy_id").val(deploy_id)

	//일괄 예외처리 (영구,한시검증제외만 가능)
	//kwj 수정 . 기존 일괄 예외 요청시에만 일괄 처리 하던것. 개별이 아닐경우로 수정
//	if($("#howToWay").val() != '' && $("#howToWay").val() == 'C'){
	if($("#howToWay").val() != '' && $("#howToWay").val() != 'A'){
		//일괄 예외처리
	...
	//개별 예외처리
		...
		if($("#exec_handling_form #exception_prc_status_cd").val() == '1' && $("#exec_handling_form #exception_prc_meth_cd").val() == '1'){
			...
			if($("#exec_handling_form #exception_prc_meth_cd").val() == '2'){
				strText ="영구 검증제외를 선택하셨을 경우 성능검증을 수행하지 않습니다. 해당 프로그램에 대해<br/>영구 검증제외로 저장하시겠습니까?";
			}else if($("#exec_handling_form #exception_prc_meth_cd").val() == '3'){
				strText ="한시 검증제외를 선택하셨습니다. <br/> 해당 프로그램에 대해 한시 검증제외로<br/>저장하시겠습니까?";
			}
...
var callback_exceptionHandlingAction = function(result) {
	...
	if(result.result){
		parent.$.messager.alert('','저장이 완료 되었습니다.','info',function(){
			setTimeout(function() {
				//지표단위일 경우에만 성능검증을 수행한다.
				if($("#exec_handling_form #exception_prc_meth_cd").val() == 1){
					if(parent.parent.openMessageProgress != undefined) parent.parent.openMessageProgress("성능검증 수행중"," ");
					ajaxCall("/ExecPerformanceCheckIndex/DeployPerfChkExcptRequest/perfCheckExecute",$("#exec_handling_form"),"POST",callback_perfCheckExecute);	
				}else{
