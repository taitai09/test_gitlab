		...
		columns:
			[
				...
				[
					...
					{field:'checking_cm_cnt',title:'검증중',width:"8%",halign:"center",align:'center'},
	...
	$("#tableList1").datagrid({
		...
		[
			[
				...
				{title:'검증결과',halign:"center",align:'center',colspan:5},
			...
			[
				...
				{field:'auto_pass_cnt',title:'검증제외',width:"8%",halign:"center",align:'center'},
...
function fnSearch0(){
	...
	if ($("#submit_form0 #searchKey").val() != "") {
		if( $("#submit_form0 #search_from_deploy_request_dt").val() == "" ) {
			parent.$.messager.alert('','기준일자를 확인해 주세요.');
			return false;
		}
		if( $("#submit_form0 #search_to_deploy_request_dt").val() == "" ) {
			parent.$.messager.alert('','종료일자를 확인해 주세요.');
			return false;
		}
	}
	
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("CM 검증 현황"," ");
...
function fnSearch1(){
	...
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("프로그램 검증 현황"," ");
...
function fn_Search2(){
	...
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("지표별 부적합 현황"," ");
...
//				getDeployPerfChkDetailResult();  //체크박스 테이블
			...
			columns:[[
				...
				{field:'sql_auto_pass_cnt',title:'SQL-검증제외',width:"8%",halign:"center",align:'center'},
				{field:'sql_indc_unit_cnt',title:'SQL-지표단위',width:"6%",halign:"center",align:'center'},
				{field:'program_auto_pass_cnt',title:'프로그램-검증제외',width:"12%",halign:"center",align:'center'},
				{field:'tr_auto_pass_cnt',title:'거래-검증제외',width:"6%",halign:"center",align:'center'}
			]],
	    	onLoadError:function() {
	    		parent.$.messager.alert('검색 에러','데이터 조회 중에 에러가 발생하였습니다.','error');
	    	}
...
function fnSearch3(){
	...
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("성능 검증 예외 처리현황"," ");
...
function fn_Search4(){
	...
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("성능 검증 예외 처리 사유별 현황"," ");
