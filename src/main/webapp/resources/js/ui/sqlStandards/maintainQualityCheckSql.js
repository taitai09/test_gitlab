...
function initializeCombobox2() {
	$('#detail_form #ui_qty_chk_idt_cd').combobox({
		...
		onLoadError: function(){
			...
			$('#detail_form #ui_qty_chk_idt_cd').combobox('textbox').attr( 'placeholder' , '선택' );
		}
	});
}

function Btn_SaveSetting(){
	var idt_cd = $("#detail_form  #ui_qty_chk_idt_cd").combobox('getValue');
	if( idt_cd == "" || idt_cd == null ) {
...
function Btn_ResetField(){
	...
	$('#detail_form #dml_yn').combobox('textbox').attr( 'placeholder' , '선택' );
	$('#detail_form #ui_qty_chk_idt_cd').combobox('textbox').attr( 'placeholder' , '선택' );
	$('#detail_form #project_by_mgmt_yn').combobox('textbox').attr( 'placeholder' , '선택' );
...
function fnSearch(){
	...
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("SQL 품질점검 RULE 관리"," ");
