...
//callback 함수
var callback_ADDMReportAction = function(result) {
	if(result.result){
		var strHtml = "";
		$("#dataArea").html("");
...
function Btn_SaveClick(){
	$("#submit_form").attr("action","/ADDMReport/Save");
	$("#submit_form").submit();		
}

function Btn_PrintClick() {
   $("#dataArea").printElement({pageTitle: 'ADDM REPORT',printMode: 'popup',overrideElementCSS:true,leaveOpen:false});
}