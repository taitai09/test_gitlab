$(document).ready(function() {
	...
	$('#selectCombo').combobox({
	    ...
	    onSelect:function(rec){
	    	...
	    	$("#start_snap_id2").textbox("setValue","");
	    	$("#end_snap_id2").textbox("setValue","");
	    }	    
	});	
});
...
function Btn_OnClick(){
	...
	if($('#start_snap_id1').textbox('getValue') == ""){
		parent.$.messager.alert('','Diff1 시작 SNAP ID를 선택해 주세요.');
		return false;
	}
	
	if($('#end_snap_id1').textbox('getValue') == ""){
		parent.$.messager.alert('','Diff1 종료 SNAP ID를 선택해 주세요.');
		return false;
	}
	
	if($('#start_snap_id2').textbox('getValue') == ""){
		parent.$.messager.alert('','Diff2 시작 SNAP ID를 선택해 주세요.');
		return false;
	}
	
	if($('#end_snap_id2').textbox('getValue') == ""){
		parent.$.messager.alert('','Diff2 종료 SNAP ID를 선택해 주세요.');
		return false;
	}	
	...
	ajaxCall("/AWRDiffReport",
			$("#submit_form"),
			"POST",
			callback_AWRDiffReportAction);		
...
//callback 함수
var callback_AWRDiffReportAction = function(result) {
	if(result.result){
		var strHtml = "";
		$("#dataArea").html("");
...
function Btn_SaveClick(){
	$("#submit_form").attr("action","/AWRDiffReport/Save");
	$("#submit_form").submit();		
}

function Btn_PrintClick() {
   $("#dataArea").printElement({pageTitle: 'AWR Diff REPORT',printMode: 'popup',overrideElementCSS:true,leaveOpen:false});
}