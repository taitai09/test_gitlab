$(document).ready(function() {
	...
	$("#tableList").datagrid({
		...
		columns:[[
			...
			{field:'tablespace_name',title:'TABLESPACE_NAME',width:"15%",halign:"center",align:"left",sortable:"true"},
			{field:'space_used',title:'SPACE_USED(GB)',width:"15%",halign:"center",align:"right",sortable:"true"},
			{field:'tablespace_size',title:'TABLESPACE_SIZE(GB)',width:"15%",halign:"center",align:"right",sortable:"true"},
			{field:'space_used_percent',title:'SPACE_USED(%)',width:"15%",halign:"center",align:"right",sortable:"true"},
			{field:'threshold_percent',title:'THRESHOLD(%)',width:"15%",halign:"center",align:"right",sortable:"true"}
		]],
...
function Btn_OnClick(){
	...
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("일 예방 점검 - TABLESPACE"," "); 

	ajaxCall("/PreventiveCheck/Tablespace",
			$("#submit_form"),
			"POST",
			callback_TablespaceAction);
}

//callback 함수
var callback_TablespaceAction = function(result) {
		...
		$.messager.confirm('', "선택한 점검대상을 예외등록하시겠습니까?", function(r){
			if(r){
				for(var i = 0; i < rows.length; i++){
					exceptObjectNameArry += nullStringToBlank(rows[i].tablespace_name) + "|";
				}
				console.log("exceptObjectNameArry :"+strRight(exceptObjectNameArry,1));
