$(document).ready(function() {
	...
	$("#tableList").datagrid({
		...
		columns:[[
			...
			{field:'owner',title:'OWNER',width:"16%",halign:"center",align:"left",sortable:"true"},
			{field:'table_name',title:'TABLE_NAME',width:"18%",halign:"center",align:"left",sortable:"true"},
			{field:'tablespace_name',title:'TABLESPACE_NAME',width:"18%",halign:"center",align:"left",sortable:"true"},
			{field:'num_rows',title:'NUM_ROWS',width:"16%",halign:"center",align:"right",sortable:"true"},
			{field:'chain_cnt',title:'CHAIN_CNT',width:"16%",halign:"center",align:"right",sortable:"true"},
			{field:'chain_percent',title:'CHAIN(%)',width:"16%",halign:"center",align:"right",sortable:"true"}
		]],
...
function Btn_OnClick(){
	...
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("일 예방 점검 - CHAINED ROWS"," "); 

	ajaxCall("/PreventiveCheck/ChainedRows",
			$("#submit_form"),
			"POST",
			callback_ChainedRowsAction);
}

//callback 함수
var callback_ChainedRowsAction = function(result) {
		...
		$.messager.confirm('', "선택한 점검대상을 예외등록하시겠습니까?", function(r){
			...
			if(r){
				for(var i = 0; i < rows.length; i++){
					exceptObjectNameArry += nullStringToBlank(rows[i].owner)+"^" +nullStringToBlank(rows[i].table_name) + "|";
				}		
