...
function Btn_OnClick(){
	...
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("일 예방 점검 - INVALID OBJECT"," "); 

	ajaxCall("/PreventiveCheck/InvalidObject",
			$("#submit_form"),
			"POST",
			callback_InvalidObjectAction);
}

//callback 함수
var callback_InvalidObjectAction = function(result) {
		...
		$.messager.confirm('', "선택한 점검대상을 예외등록하시겠습니까?", function(r){
			if(r){
				for(var i = 0; i < rows.length; i++){
					exceptObjectNameArry += nullStringToBlank(rows[i].owner)+"^"+nullStringToBlank(rows[i].object_type)+"^"+nullStringToBlank(rows[i].object_name) + "|";
				}		
