$(document).ready(function() {
	...
	$("#tableList").datagrid({
		...
		columns:[[
			...
			{field:'resource_nm',title:'RESOURCE_NM',width:"22%",halign:"center",align:"left",sortable:"true"},
			{field:'max_utilization',title:'MAX_UTILIZATION',width:"22%",halign:"center",align:"right",sortable:"true"},
			{field:'limit_value',title:'LIMIT_VALUE',width:"22%",halign:"center",align:"center",sortable:"true"},
			{field:'utilization_percent',title:'UTILIZATION(%)',width:"22%",halign:"center",align:"right",sortable:"true"}
		]],
...
function Btn_OnClick(){
	...
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("일 예방 점검 - RESOURCE LIMIT"," "); 

	ajaxCall("/PreventiveCheck/ResourceLimit",
			$("#submit_form"),
			"POST",
			callback_ResourceLimitAction);
}

//callback 함수
var callback_ResourceLimitAction = function(result) {
		...
		$.messager.confirm('', "선택한 점검대상을 예외등록하시겠습니까?", function(r){
			if(r){
				for(var i = 0; i < rows.length; i++){
					exceptObjectNameArry += nullStringToBlank(rows[i].inst_id) +"^"+nullStringToBlank(rows[i].resource_nm) + "|";
				}		
