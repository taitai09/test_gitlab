$(document).ready(function() {
	...
	$("#tableList").datagrid({
		...
		columns:[[
			...
			{field:'index_column_name',title:'INDEX_COLUMN_NAME',width:"20%",halign:"center",align:"left",sortable:"true"}
		]],
...
function Btn_OnClick(){
	...
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("일 예방 점검 - FOREIGNKEYS WITHOUT INDEX"," "); 

	ajaxCall("/PreventiveCheck/ForeignkeysWithoutIndex",
			$("#submit_form"),
			"POST",
			callback_ForeignkeysWithoutIndexAction);
}

//callback 함수
var callback_ForeignkeysWithoutIndexAction = function(result) {
			...
			if(r){
				for(var i = 0; i < rows.length; i++){
					exceptObjectNameArry += nullStringToBlank(rows[i].owner) + "^"
							...
							+ nullStringToBlank(rows[i].constraint_name) + "|";
				}
