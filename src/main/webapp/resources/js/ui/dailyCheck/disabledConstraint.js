$(document).ready(function() {
	...
	$("#tableList").datagrid({
		...
		columns:[[
			...
			{field:'constraint_name',title:'CONSTRAINT_NAME',width:"20%",halign:"center",align:"left",sortable:"true"},
			{field:'constraint_type',title:'CONSTRAINT_TYPE',width:"20%",halign:"center",align:"left",sortable:"true"}
		]],
...
function Btn_OnClick(){
	...
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("일 예방 점검 - DISABLED CONSTRAINT"," "); 

	ajaxCall("/PreventiveCheck/DisabledConstraint",
			$("#submit_form"),
			"POST",
			callback_DisabledConstraintAction);
}

//callback 함수
var callback_DisabledConstraintAction = function(result) {
		...
		$.messager.confirm('', "선택한 점검대상을 예외등록하시겠습니까?", function(r){
			if(r){
				for(var i = 0; i < rows.length; i++){
					exceptObjectNameArry += nullStringToBlank(rows[i].owner) +"^"+nullStringToBlank(rows[i].table_name) +"^"+nullStringToBlank(rows[i].constraint_name) + "|";
				}
