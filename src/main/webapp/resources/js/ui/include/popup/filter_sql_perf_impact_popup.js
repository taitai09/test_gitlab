...
//	if (Left < 0) Left = 0;
	...
	$('#filter_sql_form #condition_1').combobox({
		onSelect: function(rec){
			...
			if( rec.value == "SQL_TEXT" ) {
				comboboxAddData_SqlText();
			} else {
				comboboxAddData();
			}
		},
...
function comboboxAddData() {
	$('#filter_sql_form #condition_2').combobox('loadData',[]);
	let comboData = $('#filter_sql_form #condition_2').combobox('getData');
	
	let opts = $('#filter_sql_form #condition_2').combobox('options');
	let emptyRow = {};
	emptyRow[opts.valueField] = 'NOT LIKE';
	emptyRow[opts.textField] = 'NOT LIKE';
	comboData.unshift(emptyRow);
	
	opts = $('#filter_sql_form #condition_2').combobox('options');
	emptyRow = {};
	emptyRow[opts.valueField] = 'LIKE';
	emptyRow[opts.textField] = 'LIKE';
	comboData.unshift(emptyRow);
	
	opts = $('#filter_sql_form #condition_2').combobox('options');
	emptyRow = {};
	emptyRow[opts.valueField] = 'NOT IN';
	emptyRow[opts.textField] = 'NOT IN';
	comboData.unshift(emptyRow);
	
	opts = $('#filter_sql_form #condition_2').combobox('options');
	emptyRow = {};
	emptyRow[opts.valueField] = 'IN';
	emptyRow[opts.textField] = 'IN';
	comboData.unshift(emptyRow);
	...
	emptyRow = {};
	emptyRow[opts.valueField] = '';
	emptyRow[opts.textField] = '선택';
	comboData.unshift(emptyRow);
	
	$('#filter_sql_form #condition_2').combobox('loadData',comboData);
}
function comboboxAddData_SqlText() {
	$('#filter_sql_form #condition_2').combobox('loadData',[]);
	let comboData = $('#filter_sql_form #condition_2').combobox('getData');
