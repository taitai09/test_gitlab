$(document).ready(function() {
	$('#popProjectListPop').window({
		title : "프로젝트 조회",
		top:getWindowTop(870),
		left:getWindowLeft(950)
	});
	
	$("#pop_projectList_form #projectList").datagrid({
		view: myview,
		singleSelect: true,
		onClickRow : function(index,row) {
			setProjectRowPop(row);
			
			Btn_OnClosePopup("popProjectListPop");
		},
		columns:[[
...
function Btn_SearchProjectListPop(){
	if(!formValidationCheckPopup()){
		return false;
	}
	
	$("#pop_projectList_form #currentPage").val("1");
	$("#pop_projectList_form #pagePerCount").val("20");
	
	$('#pop_projectList_form #projectList').datagrid('loadData',[]);
	
	ajaxCallSearchProjectListPop();
}

function ajaxCallSearchProjectListPop() {
	/* modal progress open */
	...
	ajaxCall("/SQLAutomaticPerformanceCheck/searchProjectList",
			$("#pop_projectList_form"),
			"POST",
			callback_searchProjectListAddTablePop);
}

//callback 함수
var callback_searchProjectListAddTablePop = function(result) {
	/* modal progress close */
	if(parent.closeMessageProgress != undefined) parent.closeMessageProgress();
	
	var dataLength = JSON.parse(result).dataCount4NextBtn;
	
	json_string_callback_common(result, "#pop_projectList_form #projectList",true);
