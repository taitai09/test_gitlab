...
function popupOpen(form){
	...
	$('#submit_form_popup [name=top_wrkjob_cd]').val( $("#detail_form #top_wrkjob_cd[type=hidden]").val() );
	setTabs();
	createPopupTable();
...
function setChk(form){
	$('#onlyIncorrectYn').checkbox({
		...
		onChange: function( checked ){
			form.find('[name=onlyIncorrectYn]').val( checked );
			getInspectSqlList(form);
		}
	});
...
function getInspectSqlList(form){
	$("#popup_tableList").datagrid("loadData", []);
...
function createPopupTable(){
	$("#popup_tableList").datagrid({
		...
		columns:[[
			...
			{field:'perf_check_result_div_nm',title:'검증결과',halign:"center",align:'center',width:'20%',styler:cellStyler}
		]],
		...
		buttons:[{
			...
			onClick: function(){
				...
				if(row){
					...
					parent.$.messager.alert('','튜닝 요청 할 SQL ID가 없습니다.');
				}
...
function ajax_gettop_Workjb_Cd(){
	ajaxCall("/perfInspectMng/getBindValue",
			$('#submit_form_popup'),
			"POST",
			callback_inspectResultDetail);
