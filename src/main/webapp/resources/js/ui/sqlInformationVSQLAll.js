...
//			{field:'snap_id',title:'SNAP_ID',halign:"center",align:'center'},
			...
			{field:'buffer_gets',title:'BUFFER_GETS',halign:"center",align:'right',formatter:comparePreviousValue},
			{field:'elapsed_time',title:'ELAPSED_TIME',halign:"center",align:'right',formatter:comparePreviousValue},			
			{field:'rows_processed',title:'ROWS_PROCESSED',halign:"center",align:'right',formatter:comparePreviousValue},
			{field:'exec_inst_id',title:'INST_ID',halign:"center",align:'right'},
			{field:'exec_time',title:'EXECUTE_TIME',halign:"center",align:'center',formatter:comparePreviousValue}			
		]],
...
function comparePreviousValue(val, row) {
	if(val === null || val == 0) {
		try {
			...
			if(previousRow.snap_id != row.snap_id) {
				return val;
			}
		} catch(error) {
...
function searchSqlPerformancePopup(){	
	...
	/* Bind Value */
//	ajaxCall("/SQLInformation/BindValueAll",
//			$("#submit_form"),
//			"POST",
//			callback_BindValueAction);
	ajaxCall("/SQLInformation/BindValueNewList",
			$("#submit_form"),
			"POST",
			callback_BindValueAction);
...
//		callback_SQLPerformHistoryAddTable);
	ajaxCall("/SQLInformation/SQLPerformNewHistory",
		$("#submit_form"),
		"POST",
		callback_SQLPerformHistoryAddTable);

	/* Tree Plan */
	ajaxCall("/SQLInformation/TreePlanAll",
			$("#submit_form"),
			"POST",
	...
	/* Grid Plan */
	ajaxCall("/SQLInformation/GridPlanAll",
			$("#submit_form"),
			"POST",
			callback_GridPlanAction);	
	
	/* OutLine */
	ajaxCall("/SQLInformation/OutLineAll",
			$("#submit_form"),
			"POST",
	...
	/* 유사SQL */
	if ( $("#sqlYnType").val() != "1") {
		ajaxCall("/SQLInformation/SimilaritySqlAll",
				$("#submit_form"),
				"POST",
...
function Btn_NextBindSearch(){
	...
	ajaxCall("/SQLInformation/BindValueNextAll",
		$("#submit_form"),
		"POST",
