...
function getTableList(){
	$('#tableList').datagrid({
		...
		columns:[[
			...
			{field:'dbio',title:'SQL 식별자(DBIO)',width:'20%',halign:'center',align:'left'},
			{field:'except_sbst',title:'예외사유',width:'15%',halign:'center',align:'left'},
			{field:'requester',title:'요청자',width:'5%',halign:'center',align:'center'},
			{field:'reg_dt',title:'등록일시',width:'10%',halign:'center',align:'center'},
...
function setDetailView(selRow){
	...
	$('#detail_form #qty_chk_idt_nm').textbox('setValue', replaceHtmlTagToNormal(selRow.qty_chk_idt_nm));
	$('#detail_form #bottom_search_wrkjob_cd').combotree('setValue', selRow.wrkjob_cd);
	$('#detail_form #dir_nm').textbox('setValue', replaceHtmlTagToNormal(selRow.dir_nm));
	$('#detail_form #dir_nm').textbox('readonly', true);
	$('#detail_form #dbio').textbox('setValue', replaceHtmlTagToNormal(selRow.dbio));
	$('#detail_form #dbio').textbox('readonly', true);
	$('#detail_form #requester').textbox('setValue', replaceHtmlTagToNormal(selRow.requester));
	$('#detail_form #except_sbst').textbox('setValue', replaceHtmlTagToNormal(selRow.except_sbst));
};
...
function Excel_Download(){
	$('#excel_submit_form').attr('action','/excelDownMaintainQualityCheckException');
...
function Btn_OnClick(){
	...
	$('#excel_submit_form [name=qty_chk_idt_cd]').val( $('#qty_chk_idt_cd').combobox('getValue') );
	$('#excel_submit_form [name=wrkjob_cd]').val( $('#search_wrkjob_cd').combobox('getValue') );
	$('#excel_submit_form [name=dbio]').val( $('#submit_form #dbio').val() );
}
