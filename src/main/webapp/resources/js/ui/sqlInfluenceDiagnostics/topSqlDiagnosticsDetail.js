$(document).ready(function() {
	...
	$("#tableList").datagrid({
		...
		columns:[[
		    ...
		    {field:'before_perf_expect_no',hidden:'true',rowspan:2},
		    {field:'sql_id',title:'SQL ID',halign:"center",align:'center',sortable:"true",rowspan:2},
		    ...
		    {field:'before_elapsed_time',title:'ELAPSED_TIME',halign:"center",align:'right',formatter:getNumberFormat,sortable:"true"},		    
		    ...
		    {field:'after_elapsed_time',title:'ELAPSED_TIME',halign:"center",align:'right',formatter:getNumberFormat,sortable:"true"},
...
//	$('label').addClass("text-label2");

//	var checked = $("input[name='chkPlanChangeYn']").parent().find('span').eq(0).hasClass('checkbox-checked');
//	if(checked) $("#plan_change_yn").val("Y"); 
//	else $("#plan_change_yn").val("N");
	...
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("DB변경 SQL영향도 진단 상세"," "); 
...
//callback 함수
var callback_TOPSQLDiagnosticsDetailActionAddTable = function(result) {
	json_string_callback_common(result,'#tableList',true);
	$("#tableList").parent().find(".datagrid-body td" ).css( "cursor", "default" );
};

function ajaxCallTOPSQLDiagnostics(){
	ajaxCall("/TOPSQLDiagnostics/DetailAction",
			$("#submit_form"),
			"POST",
			callback_TOPSQLDiagnosticsDetailActionAddTable);	
}
...
function goList(){
	...
	$("#submit_form #menu_nm").val("DB변경 SQL영향도 진단");
	$("#submit_form").submit();	
}
...
function SqlProfileUpdate(){
	...
	ajaxCall("/TOPSQLDiagnostics/UpdateSQLProfile",
			$("#submit_form"),
			"POST",
