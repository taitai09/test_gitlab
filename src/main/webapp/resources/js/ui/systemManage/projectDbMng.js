...
$(document).ready(function() {
	...
	$('#detail_form #perf_check_target_dbid').combobox({
		...
		onSelect:function(rec){
			//$("#detail_form #parsing_schema_name").combobox('loadData',[]);
...
function createParsingSchemaNameCombo(){
	$("#detail_form #parsing_schema_name").combobox({
		url:"/Common/getUserName?dbid="+perfCheckTargetDbid,
		method:"get",
		valueField:'username',
		textField:'username',
		...
		onLoadError: function(){
			parent.$.messager.alert('','Parsing Schema Name 조회중 오류가 발생하였습니다.');
			parent.$.messager.progress('close');
		}
	});		
}
...
function createList() {
	$("#tableList").datagrid({
		...
		columns:[[
			...
			{field:'perf_check_original_db_name',title:'성능점검원천 DB',halign:'center',align:'center'},
			{field:'perf_check_target_db_name',title:'성능점검대상 DB',halign:'center',align:'center'},
			{field:'parsing_schema_name',title:'Parsing Schema Name',halign:'center',align:'center'},
			{field:'perf_check_target_yn',title:'SQL성능자동점검 대상 여부',halign:'center',align:'center'},
		...
		onSelect:function(index,row) {
			...
			$("#detail_form #parsing_schema_name").combobox('setValue',row.parsing_schema_name);
			$("#detail_form #perf_check_target_yn").combobox('setValue',row.perf_check_target_yn);
...
//			$("#detail_form #parsing_schema_name").combobox('readonly',true);
//			$("#detail_form #perf_check_target_yn").combobox('readonly',true);
...
function Btn_SaveSetting(){
	...
	if($("#detail_form #parsing_schema_name").combobox('getValue') == ""){
		parent.$.messager.alert('경고','Parsing Schema Name을 선택해 주세요','warning');
		return false;
	}	
	if($("#detail_form #perf_check_target_yn").combobox('getValue') == ""){
		parent.$.messager.alert('경고','성능점검 대상 여부를 선택해 주세요','warning');
		return false;
	}	
...
function fn_ResetField(){
	...
	$("#detail_form #parsing_schema_name").combobox({readonly:false});
	$("#detail_form #perf_check_target_yn").combobox({readonly:false});

	$("#detail_form #perf_check_original_dbid").combobox('setValue','');
	$("#detail_form #perf_check_target_dbid").combobox('setValue','');
	$("#detail_form #parsing_schema_name").combobox('loadData',[]);
	$("#detail_form #parsing_schema_name").combobox('setValue','');
	$("#detail_form #perf_check_target_yn").combobox('setValue','');
