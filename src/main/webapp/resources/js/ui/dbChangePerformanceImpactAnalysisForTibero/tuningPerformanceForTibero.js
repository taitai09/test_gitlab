	...
	$('#submit_form #project_id_cd').combobox({
		...
		onSelect: function(data) {
			...
			$('#submit_form #sqlAutoPerfCheckId').combobox({
				url:"/DBChangePerformanceImpactAnalysisForTibero/getSqlPerfPacName?isAll=Y&project_id="+data.project_id
...
function createList() {
	...
	$("#tableList").datagrid({
		...
		columns:[[
			...
			{title:'ORACLE',halign:'center',align:'center',colspan:3},
...
function ajaxCallTableList() {
	...
	/* 튜닝실적 리스트 */
	ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/loadTuningPerformanceList",
			$("#submit_form"),
			"POST",
...
function ajaxCallTableDetailList() {
	...
	/* 튜닝실적 상세 리스트 */
	ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/loadTuningPerformanceDetailList",
			$("#submit_form"),
			"POST",
...
function Excel_Download() {
	...
	if ( $("#sqlAutoPerfCheckId").combobox("getValue") == null || $("#sqlAutoPerfCheckId").combobox("getValue") == '' ){
		...
		$("#submit_form").attr("action","/DBChangePerformanceImpactAnalysisForTibero/excelTuningDownload");
		$("#submit_form").submit();
...
function Excel_Detail_Download() {
	...
	$("#submit_form").attr("action","/DBChangePerformanceImpactAnalysisForTibero/excelTuningDetailDownload");
	$("#submit_form").submit();
	$("#submit_form").attr("action","");
}
...
function sqlPerfPacReload( projectId, sqlPerfId ) {
	var url = "/DBChangePerformanceImpactAnalysisForTibero/getSqlPerfPacName?isAll=Y&project_id="+projectId
	+"&database_kinds_cd="+$("#database_kinds_cd").val();
