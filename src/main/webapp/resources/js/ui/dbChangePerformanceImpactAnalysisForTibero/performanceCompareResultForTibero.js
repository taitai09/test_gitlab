	...
	$('#submit_form #project_id_cd').combobox({
		...
		onSelect: function(data) {
			...
			$('#submit_form #sqlPerformanceP').combobox({
				url:"/DBChangePerformanceImpactAnalysisForTibero/getSqlPerfPacName?project_id="+data.project_id
...
function createList() {
	...
	$("#tableList").datagrid({
		...
		columns:[[
			...
			{field:'asis_plan_hash_value',title:'ORACLE',width:'5%',halign:"center",align:'right'},
			{field:'tobe_plan_hash_value',title:'TIBERO',width:'5%',halign:"center",align:'right'},
			{field:'asis_executions',title:'ORACLE',width:'3%',halign:"center",align:'right',formatter:getNumberFormatNullChk,sortable:true,sorter:numberSorter},
			{field:'tobe_executions',title:'TIBERO',width:'3%',halign:"center",align:'right',formatter:getNumberFormatNullChk,sortable:true,sorter:numberSorter},
			{field:'asis_rows_processed',title:'ORACLE',width:'3%',halign:"center",align:'right',formatter:getNumberFormatNullChk,sortable:true,sorter:numberSorter},
			{field:'tobe_rows_processed',title:'TIBERO',width:'3%',halign:"center",align:'right',formatter:getNumberFormatNullChk,sortable:true,sorter:numberSorter},
			{field:'asis_elapsed_time',title:'ORACLE',width:'4%',halign:"center",align:'right',formatter:getNumberFormatNullChk,sortable:true,sorter:numberSorter},
			{field:'tobe_elapsed_time',title:'TIBERO',width:'4%',halign:"center",align:'right',formatter:getNumberFormatNullChk,sortable:true,sorter:numberSorter},
			{field:'asis_buffer_gets',title:'ORACLE',width:'4%',halign:"center",align:'right',formatter:getNumberFormatNullChk,sortable:true,sorter:numberSorter},
			{field:'tobe_buffer_gets',title:'TIBERO',width:'4%',halign:"center",align:'right',formatter:getNumberFormatNullChk,sortable:true,sorter:numberSorter},
			{field:'asis_fullscan_yn',title:'ORACLE',width:'4%',halign:"center",align:'center'},
			{field:'tobe_fullscan_yn',title:'TIBERO',width:'4%',halign:"center",align:'center'},
			{field:'asis_partition_all_access_yn',title:'ORACLE',width:'5%',halign:"center",align:'center'},
			{field:'tobe_partition_all_access_yn',title:'TIBERO',width:'5%',halign:"center",align:'center'}
		]],
...
function openExplainPlan(plan_change_yn) {
	...
	$("#tabs").tabs('update',{
		...
		options:{
			title:'ORACLE Plan' //'ASIS Plan'
		}
	});
	$("#tabs").tabs('update',{
		tab: $("#tabs").tabs('getTab',2),
		options:{
			title:'TIBERO Plan'//'TOBE Plan'
		}
	});
	...
	$("#sqlIdCopy").attr("style","width:120px !important;");
	$("#sqlIdCopy").linkbutton({
		text:"ORACLE SQL ID 복사"
	});
	
	document.getElementById("tiberoSqlIdCopy").style.display = "inline-block";
	
	$("#asisPlanCopy").linkbutton({
		text:"ORACLE PLAN 복사"
	});
	
	$("#operPlanCopy").linkbutton({
		text:"TIBERO PLAN 복사"//"TOBE PLAN 복사"
	});
...
function loadExplainPlanNew( submit_form ) {
	...
	$.ajax({
		url:"/DBChangePerformanceImpactAnalysisForTibero/getSqlPerfDetailInfo",
		data:{
	...
	ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/loadExplainInfoBindValue",submit_form, "POST", callback_BindValueListNewAction);
	
	/* Tobe Plan */
	setTimeout(function() {
		let url = "";
		PlanHashValueOld = $("#submit_form #plan_hash_value").val();
		$("#submit_form #plan_hash_value").val( tobePlanHashValue );
		...
		if ( (dmlExecYn == "N" && sqlCommend != "SELECT" ) ||
			...
			url = "/DBChangePerformanceImpactAnalysisForTibero/loadAfterDMLTextPlanListAll";
		} else { 
			url = "/DBChangePerformanceImpactAnalysisForTibero/loadAfterSelectTextPlanListAll";
		}
		...
		$("#submit_form #plan_hash_value").val( PlanHashValueOld );
		PlanHashValueOld = ""; 
	},500 );
...
//	ajaxCall("/Common/getAsisPlanCompareResult"
	ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/getPlanCompareResultForTibero"
...
function ajaxCallChartData() {
	ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/loadPerfChartData",
			$("#submit_form"),
			"POST",
...
function ajaxCallTableList() {
	...
	ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/loadPerformanceResultList",
			$("#submit_form"),
			"POST",
...
function Btn_LoadPerfResultCount() {
	/* modal progress open */
	ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/loadPerfResultCount",
			$("#submit_form"),
			"POST",
...
function Excel_Download() {
	...
	let src = "/DBChangePerformanceImpactAnalysisForTibero/excelDownload";
...
function loadPerformanceResultCount() {
	...
	ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/getPerformanceResultCount",
			$("#submit_form"),
			"POST",
...
function Btn_SaveTuningDesignation(){
	...
	if ( $("#assign_form #isAll").val() == "A" ) {
		...
		/* 전체시 List 재조회 */
		ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/loadPerformanceResultListAll",
				$("#submit_form"),
				"POST",
...
function dataListCall( data ) {
	...
	if ( data.length > 0 ) {
		...
		ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/Popup/InsertTuningRequest",
				$("#assign_form"),
				"POST",
...
function Btn_SetSQLmemo() {
	...
	// sql 검토결과조회
	ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/Popup/getSqlMemo",
			$("#memo_form"),
			"POST",
			callback_getSqlMemo);
		...
		parent.$.messager.confirm( param,msgStr,function(r) {
			if (r) {
				...
				if ( totalByte > maxByte ) {
					...
					// sql 검토결과저장
					ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/Popup/updateSqlMemo",
							$("#memo_form"),
							"POST",
...
function Btn_memoDelete() {
	if ( $("#review_sbst").val() == null || $("#review_sbst").val() == '' ) {
		...
		parent.$.messager.confirm( param,msgStr,function(r) {
			if (r) {
				// sql 검토결과삭제
				ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/Popup/deleteSqlMemo",
						$("#memo_form"),
						"POST",
	...
	if ( rows.length <= 0 ) {
		...
		parent.$.messager.confirm( param,msgStr,function(r) {
			if (r) {
				...
				ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/performanceCompareReCall",
						$("#submit_form"),
						"POST",
...
function copy_to_TiberoSqlId() {
	$.ajax({
		url:"/DBChangePerformanceImpactAnalysisForTibero/getTiberoSqlId",
		data:{
			projectId : $("#submit_form #project_id_cd").combobox("getValue") ,
			sqlAutoPerfCheckId : $("#submit_form #sqlPerformanceP").combobox("getValue") ,
			sqlId : $('#loadExplainPlan_form [name = sql_id]').val(),
			planHashValue : $('#loadExplainPlan_form [name = tobe_plan_hash_value]').val()
		},
		type:"POST",
		dataType:"json",
		success: function(result) {
		...
		error: function( result ) {
			parent.$.messager.alert('', result.message );
//			alert("error?? == "+ $("#submit_form #project_id_cd").combobox("getValue") +" , "+
//			$("#submit_form #sqlPerformanceP").combobox("getValue") +" , "+
//			$('#loadExplainPlan_form [name = sql_id]').val() +" , "+
//			$('#loadExplainPlan_form [name = plan_hash_value]').val() );
		}
	});
}
