...
function createList() {
	$("#tableList").datagrid({
		...
		columns:[[
			...
			{field:'original_db_name',title:'ORACLE DB<br>(원천DB)',width:'4%',halign:'center',align:'left',rowspan:2},
			{field:'perf_check_target_db_name',title:'TIBERO DB<br>(목표DB)',width:'4%',halign:'center',align:'left',rowspan:2},
...
function showSaveSQLPerformance_popup() {
	...
	if ( rows.length > 0 ) {
		...
		if ( sqlAutoId != "" && sqlAutoId != null ) {
			...
			$.ajax({
				url:"/DBChangePerformanceImpactAnalysisForTibero/getSqlPerformanceInfo",
				data:{
...
function Btn_SaveSqlPerfPac() {
	...
	if ( $("#popup_form #project_id").val() == "" || $("#popup_form #sqlPerformanceP_cd").combobox("getValue") == "" ) {
		ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/insertSqlPerformanceInfo",
			$("#popup_form"),
			"POST",
			callback_SaveSqlPerfPac);
	} else {
		ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/updateSqlPerformanceInfo",
			$("#popup_form"),
			"POST",
			callback_SaveSqlPerfPac);
	}
...
function getMaxSqlCheckId( projectId ) {
	...
	$.ajax({
		url:"/DBChangePerformanceImpactAnalysisForTibero/getMaxSqlCheckId?project_id="+projectId,
		type:"POST",
		dataType:"json",
		success:function(result) {
...
function ajaxCallTableList() {
	...
	/* SQL점검팩 리스트 */
	ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/loadSqlPerformancePacList",
			$("#submit_form"),
			"POST",
	...
	if ( result.txtValue == "true" ) {
		...
		parent.$.messager.confirm( param, msg ,function(r) {
			if (r) {
				...
				// SQL점검팩 삭제
				ajaxCall("/DBChangePerformanceImpactAnalysisForTibero/deleteSqlAutoPerformanceChk",
						$("#submit_form"),
						"POST",
