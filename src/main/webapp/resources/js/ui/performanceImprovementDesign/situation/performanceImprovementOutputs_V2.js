$(document).ready(function() {
	...
	// 튜닝완료사유
	$('#submit_form #tuning_complete_why_cd').combobox({
		url:"/Common/getCommonCode?isAll=Y&grp_cd_id=1008",
		method:"get",
		valueField:'cd',
		textField:'cd_nm',
		onLoadSuccess:function(){
			$('#submit_form #tuning_complete_why_cd').combobox("setValue","");
			
			let code =  $("#tuning_complete_why_code").val();
			if ( code != null && code != "" ) {
				$('#submit_form #tuning_complete_why_cd').combobox('setValue', $("#tuning_complete_why_code").val() );
			}
		},
		onLoadError: function(){
			parent.$.messager.alert('','DB 조회중 오류가 발생하였습니다.');
	...
	$("#tableList").datagrid({
		...
		columns:[[
			...
			{field:'tuning_complete_why_nm',title:'튜닝 완료 사유',halign:"center",align:'center',sortable:"true", rowspan:2},
			{field:'tuning_end_why_nm',title:'튜닝 종료 사유',halign:"center",align:'center',sortable:"true", rowspan:2},
			...
			{field:'wrkjob_mgr_id',title:'업무담당자ID',halign:"center",align:'center',sortable:"true", rowspan:2},
			{field:'wrkjob_mgr_nm',title:'업무담당자명',halign:"center",align:'center',sortable:"true", rowspan:2},
			{field:'wrkjob_mgr_wrkjob_nm',title:'업무명',halign:"center",align:'center',sortable:"true", rowspan:2},
...
function ajaxCallPerformanceImprovementOutputs_V2(){
	/* modal progress open */
	if(parent.openMessageProgress != undefined) parent.openMessageProgress("성능개선결과 산출물"," ");
	
	ajaxCall("/PerformanceImprovementOutputsAction_V2", $("#submit_form"), "POST", callback_PerformanceImprovementOutputsAddTable);
}
...
function Btn_OnClick(){
	...
	ajaxCallPerformanceImprovementOutputs_V2();
}
...
function Excel_Download(){
	...
	$("#submit_form").attr("action","/PerformanceImprovementOutputs/ExcelDown_V2");
	$("#submit_form").submit();
...
function Btn_DownloadOutputsAll(){
	if ( $("#isAll").val() != null && $("#isAll").val() == "A" ) {
		ajaxCall("/PerformanceImprovementOutputsListAll_V2", $("#submit_form"), "POST", callback_PerformanceImprovementOutputsListAll );
	} else {
