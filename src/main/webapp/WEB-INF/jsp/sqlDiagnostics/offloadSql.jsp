...
<head>
    ...
    <script type="text/javascript" src="/resources/js/ui/sqlDiagnostics/offloadSql.js?ver=<%=today%>"></script>
		...
		<div class="easyui-panel searchArea" data-options="border:false" style="width:100%">
			...
			<div class="well">
				<form:form method="post" id="submit_form" name="submit_form" class="form-inline">
					...
					<input type="hidden" id="dbid" name="dbid" value="${offloadSql.dbid}"/>
					<input type="hidden" id="sql_id" name="sql_id"/>
					<input type="hidden" id="plan_hash_value" name="plan_hash_value"/>
					<input type="hidden" id="choice_id" name="choice_id" value="D"/>
<!-- 					<input type="hidden" id="tableName" name="tableName" value="FULLSCAN_SQL"/> -->
					<input type="hidden" id="tableName" name="tableName" value="OFFLOAD_SQL"/>
					<label>DB</label>
					<select id="selectCombo" name="selectCombo" data-options="editable:false" required="true" class="w130 easyui-combobox"></select>
					<label>기준일</label>
					<input type="text" id="strStartDt" name="strStartDt" value="${offloadSql.strStartDt}" data-options="panelHeight:'auto',editable:false" required="true" class="w130 datapicker easyui-datebox"/> ~
					<input type="text" id="strEndDt" name="strEndDt" value="${offloadSql.strEndDt}" data-options="panelHeight:'auto',editable:false" required="true" class="w130 datapicker easyui-datebox"/>
		...
		<div class="easyui-panel" data-options="border:false" style="width:100%;min-height:650px">
			<div id="sqlTabs" class="easyui-tabs" data-options="plain:true,fit:true,border:false">
				<div title="OFFLOAD 비효율 SQL" style="padding:5px;">
