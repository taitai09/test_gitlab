...
<html lang="ko">
<head>
	<title>성능 검증 :: 실행 SQL 성능 검증 :: 성능 검증 관리</title>
	<meta charset="utf-8" />
	...
	<script type="text/javascript" src="/resources/js/ui/execSqlPerfCheck/perfInspectMng.js?ver=<%=today%>"></script>
	...
	<style>
		#container #contents.perfInspectMng .searchAreaMulti{
			height: 95px;
		}
	</style>
</head>
...
<div id="container">	
	...
	<div id="contents" class="perfInspectMng"> 
		<div class="easyui-panel searchAreaMulti" data-options="border:false" style="width:100%">
			<div class="well">
				...
				<input type="hidden" id="perf_check_result_div_cd" name="perf_check_result_div_cd" value=""/>
				...
				<div>
					<label>업무</label>
					<select id="wrkjob_cd" data-options="panelHeight:'200px',editable:false,required:true" class="w100 easyui-combobox" required="true"></select>
					<input type="hidden" name="wrkjob_cd" value="">
					
					<label>배포요청일자</label>
					<input type="text" id="deploy_request_dt_1" value="${performanceCheckMng.deploy_request_dt_1}" data-options="panelHeight:'auto',editable:false,required:true" required="true" class="w100 datapicker easyui-datebox"/> ~
					<input type="text" id="deploy_request_dt_2" value="${performanceCheckMng.deploy_request_dt_2}" data-options="panelHeight:'auto',editable:false,required:true" required="true" class="w100 datapicker easyui-datebox"/>
					<input type="hidden" name="deploy_request_dt_1" value="">
					<input type="hidden" name="deploy_request_dt_2" value="">
					
					<label>검증상태</label>
					<select id="deploy_check_status_cd" data-options="panelHeight:'auto',editable:false" class="w100 easyui-combobox"></select>
					<input type="hidden" name="deploy_check_status_cd" value="">
					
					<label>완료방법</label>
					<select id="perf_check_complete_meth_cd" data-options="panelHeight:'auto',editable:false" class="w100 easyui-combobox"></select>
					<input type="hidden" name="perf_check_complete_meth_cd" value="">
					
					<label>최종검증단계</label>
					<select id="last_perf_check_step_id" value="${performanceCheckMng.last_perf_check_step_id}" data-options="panelHeight:'auto',editable:false" class="w150 easyui-combobox"></select>					
					<input type="hidden" name="last_perf_check_step_id" value="">
					
					<label>검증결과통보</label>
					<select id="check_result_anc_yn" value="${performanceCheckMng.check_result_anc_yn}" data-options="panelHeight:'auto',editable:false" class="w80 easyui-combobox">
						<option value="">전체</option>
						<option value="Y">통보</option>
						<option value="N">미통보</option>
					</select>
					<input type="hidden" name="check_result_anc_yn" value="">
					
				</div>
				<div class="multi">
					<sec:authorize access="!hasAnyRole('ROLE_DEV')">
						<label>배포요청자</label>
						<input type="text" id="deploy_requester_nm" value="${performanceCheckMng.deploy_requester_nm}" class="w150 easyui-textbox"/>
						<input type="hidden" name="deploy_requester_nm" value="">
					</sec:authorize>
				
					<label>배포ID</label>
					<input type="text" id="deploy_id" value="${performanceCheckMng.deploy_id}" class="w150 easyui-textbox"/>
					...
					<div class="searchBtn innerBtn">
						<sec:authorize access="hasAnyRole('ROLE_DEV','ROLE_DEV_DEPLOYMANAGER','ROLE_DEPLOYMANAGER')">
							<a href="javascript:;" id="perfChkRsltNoti" class="w120 easyui-linkbutton" data-options="disabled:false"><i class="btnIcon far fa-bell fa-lg fa-fw"></i> 검증결과 통보</a>
						</sec:authorize>
						<sec:authorize access="hasAnyRole('ROLE_DEPLOYMANAGER')">
							<a href="javascript:;" id="perfChkForceFinish" class="w120 easyui-linkbutton" data-options="disabled:false"><i class="btnIcon fas fa-stop-circle fa-lg fa-fw"></i> 강제 검증완료</a>
						</sec:authorize>
						<a href="javascript:;" class="w120 easyui-linkbutton" onClick="ReExecution();"><i class="btnIcon fas fa-redo-alt fa-lg fa-fw"></i> 성능검증 재수행 </a>
		...
		<div class="easyui-layout" data-options="border:false" style="width:100%;height:575px">
			<div data-options="region:'north',split:false,collapsible:false,border:false" style="height:310px;">
