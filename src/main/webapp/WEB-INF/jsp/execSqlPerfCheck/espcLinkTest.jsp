...
<script>
	...
	var espcip="<spring:eval expression="@defaultConfig['espcip']"/>";
	var espcport="<spring:eval expression="@defaultConfig['espcport']"/>";
	var url1="<%=url1%>";
	var status;
...
// 		});
	...
	//p_resultyn:Y
	function cmRequest1(){
		var url = "http://"+espcip+":"+espcport+"/wbn/Wbn9001I.jsp?systemgb=WBN0041007&p_servicegb=1001&p_cmid=SKI180023&p_resultyn=Y";
		console.log("url==>"+url);
		document.location.href=url;
		//ajaxCall(url, $("#submit_form1"), "GET", callback_test);
	}
	function cmSync1(){
		var url = "http://"+espcip+":"+espcport+"/wbn/Wbn9001I.jsp?systemgb=WBN0041007&p_servicegb=1002";
		console.log("url==>"+url);
		document.location.href=url;
		//ajaxCall(url, $("#submit_form1"), "GET", callback_test);
	}
	...
	function cmCall(status){
		...
		this.openMessageProgress("잠시만 기다려 주세요.", "성능 검증 중");
...
<body style="padding-left:10px;">
	...
	<div style="font-size:2.5em">실행 SQL 성능 검증(ESPC) 연동 TEST</div>
	<br/>
...
<!-- 			</tr> -->
			...
			<tr>
				<td>CM번호</td>
				<td>
					<input type="text" class="text_style" name="cmid" value="SKI0100101N0001" />
					<input type="hidden" name="p_cmid" id="p_cmid" value="SKI0100101N0001" />
					<input type="hidden" name="status" id="status" value="1" />
				</td>
			</tr>
			<tr>
				<td>CM패키지명</td>
				<td><input type="text" class="text_style" name="cmpknm" value="KB 카드 배포 패키지_Paste_the_CMID" /></td>
			</tr>
			<tr>
				<td>CM예정년월일(CM배포예정일)</td>
<!-- 				<td><input type="text" name="cmdepday" value="20181010" /></td> -->
				<td><input type="text" class="text_style" id="cmdepday" name="cmdepday" /></td>
			</tr>
			<tr>
				<td>CM담당자번호</td>
				<td><input type="text" class="text_style" name="cmusrnum" value="dev" /></td>
			</tr>
			<tr>
				<td>업무코드(WRKJOB_DIV_CD)</td>
				<td><input type="text" class="text_style" name="jobcd" value="SKI" /></td>
			</tr>
			<tr>
				<td>CM생성일시(검증서버 반영일시)</td>
<!-- 				<td><input type="text" name="cmcreateday" value="20181010101010" /></td> -->
				<td><input type="text" class="text_style" id="cmcreateday" name="cmcreateday" /></td>
			</tr>
<!-- 			<tr> -->
<!-- 				<td>상태</td> -->
...
<!-- 			</tr> -->
		...
		<div style="margin-top:10px;border-top:#ffaa55 5px solid;width:calc(100% - 10px);"></div>
		<table>
			<tr>
				<td colspan="2" style="padding-top:10px;">
					<table>
						...
						<tr>
							<td>테스트결과 : </td>
							<td><font color="red"><b><span id="testResult1" style="background-color:yellow;"></span></b></font></td>
						</tr>
					</table>
				</td>
			</tr>
...
<!-- 		<input type="button" name="postTest" id="postTest" value="POST Test" /> -->
		<input type="button" name="cmConfirm" id="cmConfirm" value="CM확정" class="btn "/>
 		<input type="button" name="cmComplete" id="cmComplete" value="CM완료(상태값 3)" class="btn" /> 
 		<input type="button" name="cmCancel" id="cmCancel" value="CM취소(상태값 4)" class="btn" /> 
 		<input type="button" name="cmDelete" id="cmDelete" value="CM삭제(상태값 5)" class="btn" /> 
		<br/>
 		<input type="button" name="cmRequest1" id="cmRequest1" value="성능검증 결과 통보1" class="btn" /> 
 		<input type="button" name="cmSync1" id="cmSync1" value="동기화1" class="btn" /> 
		<input type="button" name="btn_test" id="btn_test" value="CONFIRM_TEST_WITH_JSON" class="btn" style="width:268px;"/> 
		<div id="resultDiv1"></div>
	</form>
	<br/>
	<div style="margin-top:10px;background:linear-gradient(to bottom, #70DE9E, white);height:50px;width:calc(100% - 10px);"></div>
	<br/>
	<div style="font-size:2.5em">성능검증 결과 통보/동기화</div>
...
<!-- 		<input type="button" name="cmRequest2" id="cmRequest2" value="성능점검 결과 통보" /> -->
		<input type="button" name="cmRequest2" id="cmRequest2" value="성능검증 결과 통보" class="btn" /> 
