...
<html lang="ko">
<head>
	<title>성능 검증 :: 성능 검증 단계 관리 :: 성능 검증 단계 관리</title>
	<meta charset="utf-8" />
	...
	<script type="text/javascript" src="/resources/js/ui/execSqlPerfCheck/setting/deployPerfChkStep.js?ver=<%=today%>"></script>
</head>
			...
			<form:form method="post" id="detail_form" name="detail_form" class="form-inline">
				...
				<table class="detailT">
					...
					<tr>
						<th>단계순서</th>
						<td>
							<input type="number" id="step_ordering" name="step_ordering" class="w150 easyui-textbox" required="required"/>
						</td>
						<th>검증 단계</th>
						<td>
							<input type="text" id="perf_check_step_nm" name="perf_check_step_nm" class="w200 easyui-textbox" required="required"/>
						</td>
						<th>성능검증자동 실행여부</th>
						<td>
							<select id="perf_check_auto_exec_yn" name="perf_check_auto_exec_yn" data-options="panelHeight:'auto',editable:false" class="w150 easyui-combobox" required="required">
								<option value=""></option>
								<option value="Y">Y</option>
								<option value="N">N</option>
							</select>
					...
					<tr>
						<th>삭제여부</th>
						<td>
							<select id="del_yn" name="del_yn" data-options="panelHeight:'auto',editable:false" class="w150 easyui-combobox" required>
								<option value=""></option>
								<option value="Y">Y</option>
								<option value="N">N</option>
							</select>
					...
					<tr>
						<th>검증 단계 설명</th>
						<td colspan="5"><textarea id="perf_check_step_desc" name="perf_check_step_desc" style="width:100%;height:80px" class="validatebox-invalid"></textarea></td>
					</tr>
				</table>
