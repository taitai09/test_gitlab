...
<head>
	...
	<script type="text/javascript" src="/resources/js/ui/mqmDesign/setting/projectModelManagement.js?ver=<%=today%>"></script>
...
<!-- 			<input type="hidden" id="project_id" name="project_id"/> -->
			...
			<!-- 이전, 다음 처리 -->
			<input type="hidden" id="currentPage" name="currentPage" value="${projectModel.currentPage}"/>
			<input type="hidden" id="pagePerCount" name="pagePerCount" value="${projectModel.pagePerCount}"/>
...
<!-- 				<input type="hidden" id="project_id" name="project_id" value=""/> -->
				<input type="hidden" id="check_target_seq" name="check_target_seq" value=""/>
				<input type="hidden" id="project_check_target_type_cd" name="project_check_target_type_cd" value=""/>
				...
				<table class="detailT">
					...
					<tr>
						<th>프로젝트</th>
						<td colspan="5">
<!-- 							<input type="text" id="project_nm" data-options="required:true" name="project_nm" class="w250 easyui-textbox"/> -->
							<select id="project_id" name="project_id" class="w350 easyui-combobox" }">
								<option value="">선택</option>
								<c:forEach items="${projectList}" var="list" >
									<option value="${list.project_id}"> ${list.project_nm}</option>
					...
					<tr>
						...
						<th>모델명</th>
						<td>
							<select id="model_nm" name="model_nm" data-options="panelHeight:'220',editable:false,required:true" class="w200 easyui-combobox"></select>						
						</td>
						<th>주제영역명</th>
						<td>
