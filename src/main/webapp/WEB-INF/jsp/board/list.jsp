...
<head>
    ...
    <script type="text/javascript" src="/resources/js/ui/board/list.js"></script>
</head>
<body>
<!-- container START -->
<div id="container">	
...
<%-- 				<span class="h3"><i class="far fa-check-square fa-lg fa-fw"></i> ${boardManagement.board_nm}</span> --%>
			...
			<div class="well">
				<form:form method="post" id="submit_form" name="submit_form" class="form-inline">
					...
					<input type="hidden" id="searchKey2" name="searchKey2" value="list"/>
				...
				<div class="searchBtn innerBtn">
					<c:choose>
						<c:when test="${board.board_mgmt_no eq '2'}">
							<a href="javascript:;" class="w120 easyui-linkbutton" onClick="Btn_InsertBoard();"><i class="btnIcon fas fa-edit fa-lg fa-fw"></i> 게시판 등록</a>
						</c:when>
						<c:otherwise>
					...
					<c:forEach items="${resultList}" var="result" varStatus="status">
						...
						<td class="ctext">
							<c:choose>
								<c:when test="${result.rnum eq 0}">
									<c:choose>
										<c:when test="${board.board_mgmt_no eq '2'}"><b>[상단게시]</b></c:when>
										<c:otherwise><b>[공지]</b></c:otherwise>
									</c:choose>
								</c:when>
						...
						<td style="cursor:pointer;padding-left:10px;" onClick="Btn_ViewBoard('${result.board_no}');">
							...
							<c:choose>
								<c:when test="${fn:length(result.title) > 30}">
									<c:out value="${fn:substring(result.title,0,30)} ..."></c:out>
								</c:when>
						...
						<c:if test="${boardManagement.file_add_yn eq 'Y'}">
							<td class="ctext">
								<c:forEach items="${result.temp}" var="temp" varStatus="status">
									<c:if test="${temp.file_ext_nm ne '' and temp.file_ext_nm ne null}">
										<a href="javascript:;" onClick="Btn_downFile('${temp.file_nm}','${temp.org_file_nm}','${result.board_no}');"><img src="/resources/images/file_icon/${fn:toLowerCase(temp.file_ext_nm)}.gif" onerror="this.src='/resources/images/file_icon/unknown.gif'" style="vertical-align:middle;"/></a>
									</c:if>
								</c:forEach>
							</td>
						</c:if>
						<td class="ctext">${result.upd_dt}</td>
						<td class="ctext">${result.upd_nm}</td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
